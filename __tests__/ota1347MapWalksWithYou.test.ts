// ⚠⚠ OTA-1347 — THE MAP WALKS WITH YOU, COURSE OR NO COURSE.
//
// Owner's device log, first session with the OTA-1344 marker: six taps EAST from
...
describe('OTA-1347 — the marker and the bar follow a free walk', () => {
  it('⚠⚠ playerGridCell advances with every free step — the marker anchor moves', async () => {
    ...
    // the OTA-1344 anchor. The marker reads playerGridCell, so it has moved.
  ...
  it('⚠⚠ a FREE walk sets the "near X" area label — no course required', async () => {
    ...
    // OTA-1348 — one step out is inside the ring of influence: "<Name> Outskirts".
