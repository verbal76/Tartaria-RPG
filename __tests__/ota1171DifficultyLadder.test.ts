// OTA-1171 — THE THREE COMBAT-FEEL LEVERS GET RUNGS ON THE LADDER.
//
// Owner, after the OTA-1167→1193 balance pass landed: *"if I'm tuning this to be normal
...
describe('OTA-1171 — the identity row still changes nothing', () => {
  it('⚠⚠ `owed` IS A MATHEMATICAL NO-OP ON ALL THREE NEW DIALS', () => {
    ...
    // it — the same guarantee ota1113DifficultySystems makes for the older dials.
...
describe('OTA-1171 — the ladder is monotonic and lands on whole rounds', () => {
  ...
  it('⚠ SALVAGE HAS NO COOLDOWN AT ALL — deliberately the pre-OTA-1170 game', () => {
  ...
  it('⚠ GEAR BLEND RISES TO EXACTLY THE FULL DESIGNED WEIGHT AT THE TOP', () => {
    // OTA-1159 wrote 1.0 as the full weight and shipped 0.5 pending device evidence.
...
describe('OTA-1171 — no tier can make the world easier than authored', () => {
  it('⚠⚠ A FRESH ARRIVAL READS EXACTLY 0 AT EVERY RUNG', () => {
    // The OTA-1159 guarantee, re-proven against the new multiplier. Both gear terms are
...
describe('OTA-1171 — custom runs can switch each lever independently', () => {
...
describe('OTA-1171 — the consumers read the dial, not the constant', () => {
  it('⚠ THE GEAR WEIGHT IS READ AT THE ONE CHOKE POINT', () => {
    ...
    // exactly how the AC term went missing for as long as it did (OTA-1159).
  ...
  it('the per-tile regen is scaled, and the STAMINA half deliberately is not', () => {
    ...
    // punishment billed as one, and OTA-1169 left it per-action on purpose.
...
describe('OTA-1171 — the amendment is on the record', () => {
  it('⚠⚠ `gearBlend` CROSSES pressure.ts\'s OWN PROHIBITION, AND SAYS SO', () => {
    ...
    // WRITTEN DOWN the way OTA-1113 recorded its reversal, not quietly worked around —
