 ...
 *
 * Plus the lesson OTA-1087's audit taught the hard way: authored is not
...
describe('OTA-1090 — the authoring holds together', () => {
  ...
  it('⚠ EVERY fork option in the game has a regard value and an echo', () => {
    // The cross-file completeness check OTA-1087's audit is the reason for. An
...
describe('OTA-1090 — the arc across the nine Cores', () => {
...
describe('OTA-1090 — the opinion is derived, and it moves', () => {
...
describe('OTA-1090 — ⚠ the opinion cannot be farmed', () => {
  ...
  it('answering every fork the best way still cannot alone reach the top band', () => {
    // OTA-1108 — the sheet now itemises one row per judged answer (the old
...
describe('OTA-1090 — what he actually names', () => {
...
describe('OTA-1090 — the remark itself', () => {
  const rich = () => {
    const p = withCores(4);
...
describe('OTA-1090 — the beats fire once, and only one at a time', () => {
...
describe('OTA-1090 — the name he has not used', () => {
  const beloved = () => {
    const p = withCores(9);
    p.titleProgress = { loreRead: 99, relicsPreserved: 99 } as never;
    p.factionStanding = [{ factionId: 'a', standing: 100 }] as never;
...
describe('OTA-1090 — where the opinion is legible', () => {
...
// ── ⚠ REACHABILITY (the OTA-1087 lesson) ─────────────────────────────────

describe('OTA-1090 — ⚠ every route into this content is real', () => {
  beforeAll(async () => {
  ...
  it('the name question is answered BEFORE any lookup that could swallow it', () => {
    ...
    // ⚠ OTA-1221 — anchored on the CALL, not its argument. This pinned
...
describe('OTA-1090 — ⚠ the persona is authored, not generated', () => {
  it('nothing in the engine module awaits, rolls, or reaches for a model', () => {
    ...
    // store. Same guard shape as OTA-1086's flourish module.
