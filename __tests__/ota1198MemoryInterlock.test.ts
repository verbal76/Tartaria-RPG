// OTA-1198 — THE FIX I SHIPPED THIS AFTERNOON WAS BUILDING A LOOP. THE INSTRUMENTS CAUGHT IT.
//
// First device log from OTA-1197, and it indicts OTA-1196 in its own words:
//
...
//
// ⚠⚠ SEVEN ~400MB ALLOCATIONS IN FORTY SECONDS, AND OTA-1196's FIX WAS THE ENGINE.
// The loop: the watchdog kicks a load → iOS fires a memory warning → the OTA-1196 handler
// disposes the context to free memory → **the dispose marks the in-flight load STALE**
// (OTA-1107's lifecycleGen) → the load settles to `'idle'` → the watchdog sees not-ready
...
// reload was not. This is the interlock that was missing, not a reversal of OTA-1196.
//
// ⚠ AND WHAT WORKED IS WORTH RECORDING TOO: the OTA-1196 ceiling bounded it at 8 and the
...
// OTA-1195's instruments none of this would have been visible at all.
...
describe('OTA-1198 — a memory warning now silences the watchdog', () => {
  ...
  it('⚠⚠ THE FLAG IS SET *BEFORE* THE DISPOSE, and the ordering is the fix', () => {
    ...
    // ⚠ OTA-1202 widened this window. The handler grew a long comment recording that its
  ...
  it('⚠ THE WATCHDOG ACTUALLY CHECKS IT, and refuses the reload', () => {
    ...
    // FIFTH fixed-size source slice to age this session — OTA-1204 restructured the two
  ...
  it('⚠ THE DISPOSE ITSELF IS NOT REVERTED — freeing under pressure is still right', () => {
    // This OTA adds an interlock; it does not undo OTA-1196. Handing memory back when the
    // OS asks is correct and stays.
    //
    // ⚠⚠ RETARGETED BY OTA-1202, AND THE NEW ASSERTION IS STRONGER THAN THE OLD ONE.
