// OTA-1142 — THE FOUR-LEVER BATCH, from the master tuning list.
//
...
// Same fixture shape as ota931: hpMax 100 → power = strength + 10.
...
describe('OTA-1142 lever 1 — first Guardian IS the base (hpMult 1.0)', () => {
...
describe('OTA-1142 lever 3 — acid shreds a boss on a +2 headroom, not +6', () => {
  it('boss cap is ACID_SHRED_MAX + 2', () => {
    expect(acidShredCap({ boss: true })).toBe(ACID_SHRED_MAX + 2);
    // ⚠ Retargeted by OTA-1150. The absolute was 7 back when ACID_SHRED_MAX was
    // 5; that batch took the BASE cap 5 → 3, so the same +2 headroom now totals
    // 5. OTA-1142's actual claim — the boss BONUS is 2, not 6 — is the relative
...
describe('OTA-1142 lever 4 — bosses fight the person in front of them', () => {
