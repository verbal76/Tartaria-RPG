...
describe('OTA-839 concepts.json — combat/stat entries match the engine', () => {
  ...
  it('any built-in damage-type concept for cold is accurate (engine: damage types are content-pack-driven, so there is no built-in cold concept — that is fine)', () => {
    // engine_Dev — unlike HAL, this line's concepts.json carries NO built-in
    // damage-type entries (damage types live in the content pack), so a `cold`
    // concept legitimately doesn't exist here. Only assert accuracy IF one is present.
    const cold = conceptById('cold');
    if (cold) expect(cold.answer).toMatch(/chill/i);
  });
