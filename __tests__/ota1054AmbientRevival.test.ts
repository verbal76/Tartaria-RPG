...
describe('OTA-1054 — SOURCE LOCKS', () => {
  ...
  it('the REACTIVE path never filters second-person openers (it narrates actions)', () => {
    ...
    // count as a use.
    // ⚠⚠ OTA-1260 CORRECTED THIS TEST'S PREMISE, and the distinction is the whole
    ...
    // and an unfiltered banked line produced a golem log's *"You climb down the
    // arch"* four rooms after the climb.
    //
    // So the assertion is now about WHERE the check sits, not whether the
    // identifier occurs: inside the `bankOnly` branch, and nowhere else here.
    ...
    expect(body.indexOf("get().appendLog('arbiter', finalText);")).toBeGreaterThan(uses[0]!);
  });
});
