// OTA-1138 — THE COMMA THAT ATE THE BEAT, and the label that lied about a
// feature working.
//
// Both findings come from the same device log that confirmed OTA-1136/1160,
...
// so OTA-1136's 280 ms sentence beat NEVER FIRED on exactly the class of line
...
// signature — the OTA-1119 watchdog's cue, the thing that once cost a week to
// chase. `preempted` is OTA-1134 working as built. The discard classifier
...
describe('OTA-1138 — ⚠ the attribution comma promotes to a full stop', () => {
  ...
  it('⚠ WHY IT MATTERS: the promoted stop is what re-arms the sentence beat', () => {
    ...
    // sentence boundary (OTA-1136). A comma is not a terminator, so the old
...
describe('OTA-1138 — ⚠ a preempted synthesis is no longer filed as empty', () => {
  ...
  it('⚠ WHY: empty is the dormancy signature and must stay clean', () => {
    // The OTA-1119 dormancy hunt keyed off `empty`. Preemptions polluting that
