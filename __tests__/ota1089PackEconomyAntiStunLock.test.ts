// OTA-1089 — pack action economy + anti-stun-lock. The OTA-1087 telemetry
...
describe('OTA-1089 — pack action economy + anti-stun-lock (live volley)', () => {
  const store = useGameStore;

  function makePack(n: number, traits: string[] = []): Enemy[] {
    const base = findEnemyByName('Gutter Rat');
    if (!base) throw new Error('ota1089: Gutter Rat missing — roster drift');
  ...
  function installPack(pack: Enemy[]): void {
    ...
    if (!scene) throw new Error('ota1089: no scene');
    store.setState({
      currentScene: {
        ...scene,
        enemies: pack,
...
describe('OTA-1089 — knockouts end fights (subdual resolution)', () => {
  const store = useGameStore;
...
describe('OTA-1089 — braced tick class + source locks', () => {
  ...
  it('bosses and ranged enemies are exempt from the swing cap', () => {
    // OTA-1093 retarget — the exemption became RANGE-AWARE (a shooter standing
