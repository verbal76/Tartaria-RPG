...
describe('WEB-006 — the viewport maths', () => {
  ...
  it('⚠⚠ WEB-006 — THE MARKER SITS ON THE ROOM, EVEN WHERE THE VIEW CLAMPS', () => {
...
describe('WEB-006 — the downscaled tiles', () => {
  ...
  it('⚠⚠ the tiles are SMALL — this is the memory decision, not a detail', () => {
    ...
    for (const t of TILES) {
      ...
      // ⚠ WEB-006 raised the tile 512 → 768 for sharpness (see the generator).
...
describe("WEB-006 — the owner's two conditions", () => {
  ...
  it('tapping it opens the Atlas', () => {
    // ⚠ the tap grew a tutorial-lock guard when WEB-006 deleted the MAP
...
describe('WEB-006 — the Atlas gestures do what the owner specified', () => {
  const map = src('app', 'screens', 'MapScreen.tsx');
  ...
  it('⚠⚠ and none of the page-origin machinery survives, because none is needed', () => {
    // WEB-006/1373 anchored on the FINGERS, which needs the box's page origin:
...
describe('WEB-006 — one control for the Atlas, not two', () => {
  const exp = src('app', 'screens', 'ExplorationScreen.tsx');
...
describe('WEB-006 — the codex opens on BEASTS, and the tutorial teaches the corner', () => {
  ...
  it('⚠⚠ and it gets its own beat, because it is now the ONLY way into the Atlas', () => {
    // WEB-006 deleted the scene bar's MAP button. A player who does not know
