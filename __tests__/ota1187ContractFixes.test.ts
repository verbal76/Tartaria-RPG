// OTA-1187 — PUNCHLIST P5, P7 and P8. Three defects from the P3 audit, none of which
...
describe('⚠⚠ OTA-1187 / P7 — the bonus measures the TRIP now', () => {
...
describe('⚠⚠ OTA-1187 / P7 — the stamp is written and read everywhere', () => {
...
describe('⚠⚠ OTA-1187 / P8 — the board takes back what it posted', () => {
  ...
  test('the faction gate reads the COUNTERPARTY, not the vendor', () => {
    ...
    // ⚠ `\w+!?\.faction` — OTA-1188 made the counterparty optional on the courier paths,
  ...
  test('⚠ crediting an NPC still requires an actual NPC', () => {
    // A board or a hall is not a person; OTA-1050's ledger must not gain a phantom entry.
...
describe('⚠⚠ OTA-1187 / P5 — the "challenges are off" comment was false', () => {
  ...
  test('⚠ THE PREMISE: they really are all on, which is why the comment was wrong', () => {
    ...
    // switch a challenge off. That is the OTA-1182 trap again: a pattern that matches
...
describe('OTA-1187 — the ownership map the P8 hall branch depends on', () => {
