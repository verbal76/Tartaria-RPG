// ⚠⚠ OTA-1279 — THE OUTPOST MAP THE OWNER DREW, ENFORCED AS A GRAPH.
//
...
describe('OTA-1279 — the universal graph itself', () => {
...
describe('OTA-1279 — the layout wears the graph', () => {
...
describe('OTA-1279 — one edge at a time, and directions when you ask for more', () => {
  ...
  it('⚠⚠ a bare room name still outranks the verb it collides with — even unvisited', () => {
    // OTA-1274 opened this door; OTA-1279 widened it. The old matcher only
    // offered rooms already visited (inherited from fast-travel), so `vault`
    // jumped and `forge` opened crafting right up until you had walked in once.
    expect(matchHubRoomName('vault', gate, null)).toBe('outpost_relic_vault');
    expect(matchHubRoomName('forge', gate, 'true_tartarians')).toBe('outpost_armory');
    // Still strict: a room name inside a longer sentence is not a walk order.
    expect(matchHubRoomName('break the door', gate, 'conspiracy_architects')).toBeNull();
  });
...
describe('OTA-1279 — there is always a way out, and it is on the spine', () => {
