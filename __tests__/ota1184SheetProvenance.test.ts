// OTA-1184 — THE SHEET SHOWS WHERE ITS NUMBERS CAME FROM.
//
...
// they received it."
//
// Three asks, one theme, and it is the same theme as OTA-1181 and OTA-1183: the game
...
describe('OTA-1184 — HP says where it came from', () => {
  ...
  it('the store awards the milestone with the constant the sheet explains it with', () => {
    ...
    // the same cleanup OTA-1179 did for JOIN_THRESHOLD.
...
describe('OTA-1184 — the Arbiter row says "gifts given" and opens', () => {
...
describe('OTA-1184 — the ledger says what, to whom, and how it landed', () => {
  const WM = {
    npcRelations: {
      ...
      odar: { name: 'Odar Flameforge', wrongs: 0, trades: 0, gifts: [
        { name: 'Core Relic', atHours: 30 },  // pre-OTA-1184: no reaction on file
      ] },
    },
  } as never;
  ...
  it('⚠ says nothing rather than guessing when the reaction predates the field', () => {
    // Inventing one would be worse than a blank: OTA-1176 rewrote the entire taste
  ...
  it('every reaction tier has a player-facing word', () => {
    ...
    // 'disliked' is NOT an insult — OTA-1176 added that tier precisely because a
