// OTA-1192 — PUNCHLIST P10 CLOSED. The Hidden Market takes back what it hands out.
//
...
describe('⚠⚠ OTA-1192 — the asymmetry is closed', () => {
  ...
  test('a roadside drifter is still not a broker', () => {
    // The OTA-1185 rule survives: four of the six factionless vendors spawn on the road,
    // and matching on a null faction would delete the travel entirely.
    expect(isContractBroker({ id: 'naha_drifter' })).toBe(false);
  });
});

describe('⚠⚠ OTA-1192 — THE LADDER, and it must hold at every rung', () => {
...
describe('⚠⚠ OTA-1192 — the rate table is the ONE source of truth', () => {
...
describe('⚠ OTA-1192 — a stall still takes its OWN faction’s work at full pay', () => {
