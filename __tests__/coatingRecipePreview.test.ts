...
describe('OTA-704 — coating output is legible on the recipe card', () => {
  it('a coating row shows its damage dice + type', () => {
    // ⚠ OTA-1573 — plain base coatings all sit on the plain-Uncommon rung now.
    expect(coatingLine('Incendiary Paste')).toBe('Coats weapon: +1d6 burn (Burning)');
    expect(coatingLine('Poison Vial')).toBe('Coats weapon: +1d6 poison (Poisoned)');
  });
  ...
  it('a higher-dice variant reads stronger (Plague Vial 1d10 > Poison Vial 1d6)', () => {
    expect(coatingLine('Poison Vial')).toContain('+1d6 poison');
    expect(coatingLine('Plague Vial')).toContain('+1d10 poison');
  });
