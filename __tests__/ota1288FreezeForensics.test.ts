...
describe('OTA-1288 (port of golem OTA-1276) — it is stamped at the doors a freeze happens behind', () => {
  ...
  it('⚠⚠ boot READS it before clearing, and says so in the log', () => {
    const store = src('app', 'state', 'gameStore.ts');
    // ⚠ OTA-1366 — the window is the try/catch, NOT a byte count. This read
    // `store.slice(i, i + 1400)`, and OTA-1366 adding the native-death promotion
    ...
    // ⚠ And the OTA-1366 crash record lands between them: recorded before the
