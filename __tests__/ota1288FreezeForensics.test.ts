...
// ⚠⚠ OTA-1288 (port of golem OTA-1276) — THE LOG CANNOT SHOW THE FREEZE, AND EVERY DIAGNOSIS THAT READ
...
describe('OTA-1288 (port of golem OTA-1276) — the breadcrumb outruns the wedge', () => {
...
describe('OTA-1288 (port of golem OTA-1276) — it is stamped at the doors a freeze happens behind', () => {
  ...
  it('⚠⚠ boot READS it before clearing, and says so in the log', () => {
    const store = src('app', 'state', 'gameStore.ts');
    // ⚠ OTA-web10 — the window is the try/catch, NOT a byte count. This read
    // `store.slice(i, i + 1400)`, and OTA-web10 adding the native-death promotion
    ...
    // ⚠ And the OTA-web10 crash record lands between them: recorded before the
...
describe('OTA-1288 (port of golem OTA-1276) — why the existing detector could never have caught this', () => {
