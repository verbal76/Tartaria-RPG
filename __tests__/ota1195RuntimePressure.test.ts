...
describe('OTA-1195 — the memory warning the owner asked for', () => {
...
describe('OTA-1195 — two clocks, because one cannot answer the question', () => {
...
describe('OTA-1195 — the app-state trail, which is evidence and not noise', () => {
...
describe('OTA-1195 — the tap breadcrumb', () => {
...
describe('OTA-1195 — the reload is timed and its outcome named', () => {
...
describe('OTA-1195 — the counts reach the bug report header', () => {
...
describe('OTA-1195 — this OTA changes no behaviour, deliberately', () => {
  ...
  it('⚠ THE HOLD IS OVER — OTA-1196 LANDED THE FIX, and this is the assertion that changed', () => {
  ...
  it('⚠ EVERY LISTENER IS GUARDED — headless and test environments have no AppState', () => {
    const i = STORE.indexOf('function startRuntimePressureWatch');
    // ⚠ Widened by OTA-1196, which added the memory-warning RESPONSE (a dispose) between
    // the listener and the frame clock. A fixed slice that stops short reads as a missing
    // guard rather than as a moved one — the ota1186 defect in a new costume.
    // ⚠ Widened again by OTA-1202, which recorded inside this handler why its own release
