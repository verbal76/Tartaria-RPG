...
// ── Fix #1 — frayed-rope fall loop ────────────────────────────────────────────
describe('OTA-625 #1 — frayed rope refuses on the ground instead of farming fall damage', () => {
  async function setupFrayed(elevated: boolean) {
    const store = useGameStore;
    await store.getState().hydrate();
    await store.getState().startNewGame({ name: 'Climber', raceId: 'unknowing_mass', factionId: 'reclaimers_guild' });
    store.getState().skipTutorial?.();
    const p0 = store.getState().player!;
    // Reclaimer's Rope worn to durability 10 (≤ ROPE_WEAR_PER_TIER = 15) — a
    // guaranteed snap on the next tier.
    const rope: InventoryItem = {
      id: 'frayed_rope', name: "Reclaimer's Rope", kind: 'misc', quantity: 1,
      tags: ['utility', 'gate'], durability: { current: 10, max: 50 },
    } as unknown as InventoryItem;
    store.setState({
      currentScene: {
        ...store.getState().currentScene!,
        ambientNouns: ['wall', 'tower'],
        elevatedOn: elevated ? { noun: 'wall', tier: 1, totalTiers: 2 } : null,
      },
      player: {
  ...
  it('on the GROUND: refuses + warns, no HP lost, no fall, rope left intact', async () => {
    const store = await setupFrayed(false);
    store.getState().submitPlayerAction('climb wall');
    const after = store.getState().player!;
    expect(after.hp).toBe(40); // no fall damage
    const logs = store.getState().gameLog.map((e) => e.text).join('\n');
    expect(logs).toMatch(/too frayed|frayed through/i);
    expect(logs).not.toMatch(/YOU FALL/);
    // Rope NOT converted to Broken Rope — still mendable.
    expect(after.inventory.some((i) => i.id === 'frayed_rope' && i.name === "Reclaimer's Rope")).toBe(true);
    expect(after.inventory.some((i) => i.name === 'Broken Rope')).toBe(false);
  });

  it('ALREADY UP: still falls (committed mid-climb) and the rope snaps to Broken Rope', async () => {
    const store = await setupFrayed(true);
    store.getState().submitPlayerAction('climb wall');
    const after = store.getState().player!;
    expect(after.hp).toBeLessThan(40); // fall damage taken
    const logs = store.getState().gameLog.map((e) => e.text).join('\n');
