...
describe('WEB-005 — the viewport maths', () => {
  ...
  it('⚠⚠ WEB-005 — THE MARKER SITS ON THE ROOM, EVEN WHERE THE VIEW CLAMPS', () => {
...
describe('WEB-005 — the downscaled tiles', () => {
  ...
  it('⚠⚠ the tiles are SMALL — this is the memory decision, not a detail', () => {
    ...
    for (const t of TILES) {
      ...
      // ⚠ WEB-005 raised the tile 512 → 768 for sharpness (see the generator).
...
describe("WEB-005 — the owner's two conditions", () => {
...
describe('WEB-005 — the Atlas gestures do what the owner specified', () => {
  const map = src('app', 'screens', 'MapScreen.tsx');
  ...
  it('⚠⚠ and none of the page-origin machinery survives, because none is needed', () => {
    // WEB-005/1373 anchored on the FINGERS, which needs the box's page origin:
