...
/**
 * OTA-1087 — CAN THE PLAYER ACTUALLY GET THERE?
 *
 ...
 * in the shipped game reaches it. So OTA-1085 shipped 44 topics for the
...
const at = (over: Partial<TalkContext> = {}): TalkContext => ({
  ...
  hasRecentRaidNews: false, chapter: 'hook', cores: 0, choices: [], ...over, // OTA-1088
});

beforeAll(() => { console.log = () => {}; console.warn = () => {}; console.error = () => {}; });

describe('OTA-1087 — the identity bug that made a whole cast mute', () => {
...
describe('OTA-1087 — every authored class set has a live route', () => {
  /* eslint-disable @typescript-eslint/no-require-imports */
  const store: string = require('fs').readFileSync(
    require('path').join(__dirname, '../app/state/gameStore.ts'), 'utf8');
  /* eslint-enable @typescript-eslint/no-require-imports */
...
describe('OTA-1087 — the buttons say different things for different people', () => {
...
describe('OTA-1087 — driving the real store', () => {
  beforeAll(async () => {
 ...
 */
describe('OTA-1087 audit — hostility cannot be farmed for rival standing', () => {
  /* eslint-disable @typescript-eslint/no-require-imports */
  ...
  it('a refused gift is metered — it costs the item nothing to offer', () => {
    // resolveGift refuses rather than consumes, by design (OTA-1083). That makes
  ...
  it('the dock records a MAGNITUDE, so a small insult cannot buy off a big one', () => {
    const fn = store.slice(store.indexOf('function dockHostileStanding('), store.indexOf("/** OTA-1083 — a gift's standing effect"));
    expect(fn).toContain('const applied = Math.max(0, magnitude - already);');
    expect(fn).toContain('standingDocked: magnitude');
  });
});

describe('OTA-1087 audit — a grant that cannot land does not spend the topic', () => {
  /* eslint-disable @typescript-eslint/no-require-imports */
  const store: string = require('fs').readFileSync(
    require('path').join(__dirname, '../app/state/gameStore.ts'), 'utf8');
  /* eslint-enable @typescript-eslint/no-require-imports */
...
describe('OTA-1087 audit — gifting reaches the same cast as talking', () => {
  /* eslint-disable @typescript-eslint/no-require-imports */
  const store: string = require('fs').readFileSync(
    require('path').join(__dirname, '../app/state/gameStore.ts'), 'utf8');
  /* eslint-enable @typescript-eslint/no-require-imports */
