...
describe('OTA-837 (1) — data contracts the codex renders', () => {
  ...
  it('the concepts bank is non-empty and every entry has a title + answer', () => {
    // engine_Dev — engine's built-in concepts bank is smaller (~84 vs HAL's 172) and
    // is NOT surfaced in engine's codex (the 837 lore tab is HAL/golem-only; engine
    // is bestiary-only, its lore being content-pack-driven). Keep the contract loose.
