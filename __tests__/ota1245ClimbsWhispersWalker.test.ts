...
// ⚠⚠ OTA-1245 — TEXAS RANGER CLIMBS THE TOWERS AND CHASES THE WHISPERS. The
...
//
// ⚠⚠ HARNESS TRAPS: the five in the ota1242HuntWalker header all apply. Two
...
describe('OTA-1245 — Texas Ranger on the Great Climbs and the whisper chains', () => {
  beforeAll(async () => {
    ...
    it(`⚠⚠ climb ${climb.id} — ${climb.tiers} tiers, summit boss falls, crest banks once, ${climb.rewardArmor} paid`, async () => {
      ...
      while (!isBossUp()) {
        ...
        if (elevNow && elevNow.tier >= elevNow.totalTiers) {
          ...
          if (!isBossUp()) {
            throw new Error(`${climb.id}: crested the tower but no summit boss appeared — if the scene shows elevatedOverlayMeta, the OTA-1245 crown-overlay guard has regressed\n`
              + store.getState().gameLog.slice(-6).map((e) => `${e.channel}: ${e.text}`).join('\n'));
          }
          break;
        }
