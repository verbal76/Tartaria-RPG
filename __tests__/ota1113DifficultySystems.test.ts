...
describe('OTA-1113 — pack size and the swing cap move together', () => {
  ...
  it('both consumers are wired, and the store says why', () => {
    ...
    expect(COMBAT_SRC).toContain('scaledSwingCap(MELEE_PACK_SWINGS_PER_ROUND, profileOf(get().player).pack)');
    expect(COMBAT_SRC).toContain('THE CAP MOVES WITH THE PACK');
  });
});
