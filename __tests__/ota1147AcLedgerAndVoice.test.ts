// OTA-1147 — MEASURE THE TWO THINGS WE KEEP GUESSING AT, AND FIX THE ONE THAT
// IS ACTUALLY FIXABLE.
//
...
// whenever it shifts by 2 or more. OTA-1132 is the precedent: instrument
...
//     OTA-1054's broader version dropped every sentence starting with "You"
...
//     exactly how OTA-1054 happened.
// What ships instead is EVIDENCE: an accepted ambient line now logs its raw
// text. OTA-1057 added that for failures and it has paid for itself since —
...
// ⚠ OTA-1148 RETARGET. OTA-1147 shipped an OPENER test and the very next
...
describe("OTA-1147 — the first-person opener, and what it must NOT eat", () => {
  it('⚠ drops narrator-only lines', () => {
    ...
    // RETARGETED BY OTA-1148 — this one contains "you", so under the corrected
  ...
  it('⚠ keeps an impersonal reflection — the shape a good musing often takes', () => {
    // OTA-1054's lesson in one assertion: a filter that looks obviously right
...
describe('OTA-1147 — an accepted ambient line leaves a trace', () => {
  ...
  it('⚠ the scenery half is recorded as deliberately UNFIXED', () => {
    ...
    // not a filter, and guessing at one is how OTA-1054 happened.
    expect(SRC).toContain('DELIBERATELY NOT "FIXED" HERE');
    expect(SRC).toContain('whitelist of what may exist in each biome');
  });
});

describe('OTA-1147 — the AC ledger prints a derivation, not a number', () => {
...
describe('OTA-1147 — the pinned surface chip stops lying on a perch', () => {
...
describe('OTA-1148 — the filter now catches the line from the device log', () => {
  ...
  it('⚠ and it STILL cannot eat the feature — OTA-1054 is the standing warning', () => {
