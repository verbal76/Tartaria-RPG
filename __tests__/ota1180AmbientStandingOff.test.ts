// OTA-1180 — THE WORLD DOES NOT MOVE YOUR STANDING.
//
// Owner, 2026-08-07, deciding the first of the four design calls OTA-1179 held:
...
//
// These assertions are the inverse of the hold that OTA-1179 wrote. That one
...
describe('OTA-1180 — no ambient event grants standing', () => {
  ...
  it('the two events that used to pay standing now pay a tide instead', () => {
    ...
    // with nothing behind it — OTA-1179 finding 9, re-introduced on purpose. Both
...
describe('OTA-1180 — the attack gate is untouched', () => {
  ...
  it('the hostile threshold and the hunt roll are unchanged', () => {
    ...
    // and OTA-1179's per-faction fix is still in place
    const store = read('app', 'state', 'gameStore.ts');
