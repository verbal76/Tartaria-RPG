// OTA-1193 — PUNCHLIST P12 CLOSED. An ambiguous contract name refuses instead of guessing.
//
// ⚠⚠ THE DEFECT. All four contract finders resolve a typed title in tiers: exact, then
// substring, then (since OTA-1188) token match. The substring tier used `pool.find()`,
...
//
// ⚠ WHY IT WAS NOT FIXED IN OTA-1188, where it was found. That change dropped a shared
...
describe('⚠⚠ OTA-1193 — ambiguity refuses at EVERY tier now', () => {
...
describe('⚠⚠ OTA-1193 — and it did not become useless', () => {
  ...
  test('the token tier still rescues the parser’s stop-word stripping', () => {
    // The OTA-1188 case that started all of this: the parser hands over
...
describe('⚠ OTA-1193 — the real catalogs did not lose anything', () => {
