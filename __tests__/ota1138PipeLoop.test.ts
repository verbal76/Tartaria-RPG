// OTA-1138 — THE PIPES WERE THE BUG.
//
// Three OTAs have now been spent on item synthesis. OTA-1131 made the silent
// failure visible. OTA-1132 shrank the prompt, raised the cap 180 → 240, and —
...
describe('OTA-1138 — no alternation survives inside the JSON', () => {
  ...
  it('every effect example in the brief is itself valid JSON with literal values', () => {
    // ⚠ RETARGETED BY OTA-1157, and the reason is the point of that OTA. The
...
describe('OTA-1138 — the vocabulary the validator needs is still fully specified', () => {
...
describe('OTA-1138 — the loop now has a name in the telemetry', () => {
  ...
  it('the raw sample OTA-1132 added is still carried — that is what caught this', () => {
    expect(mod).toContain('raw.trim().replace(/\\s+/g, \' \').slice(0, 160)');
    expect(mod).toContain('(${raw.length}ch)');
  });
});

describe('OTA-1138 — the file records why the two earlier fixes could not have worked', () => {
  ...
  it('the cap OTA-1132 raised is left alone — it was never the constraint', () => {
