...
// OTA-975 — zero grip means gravity. Owner (from the pillar log): "you
...
describe('OTA-975 — fall math is shared and unchanged', () => {
...
describe('OTA-975 — zero on the wall: one warning, then gravity', () => {
...
describe('OTA-975 — eating mid-climb is the ungated escape hatch', () => {
...
describe('OTA-975 — rest-on-wall refusal answers every retry', () => {
  it('two rests on the wall get two visible refusals (no dedup silence)', async () => {
    ...
    store.getState().submitPlayerAction('rest');
    await new Promise((r) => setTimeout(r, 5));
    // OTA-1039 — match EITHER wall-rest refusal (the rope-specific one added this
