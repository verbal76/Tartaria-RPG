...
// OTA-938 — one-time dead/abandoned-dog revive migration (fires exactly once ever) +
...
describe('OTA-938 — one-time dog revive', () => {
  ...
  it('revives a DEAD dog once, at full HP, and latches the migration', () => {
    const withDeadDog = { ...base, dog: deadDog(), dogRevivedOta938: undefined } as PlayerCharacter;
    ...
    expect(out.dogRevivedOta938).toBe(true);
  });

  it('does NOT re-revive a dog that dies AFTER the latch is set', () => {
    const alreadyRun = { ...base, dog: deadDog(), dogRevivedOta938: true } as PlayerCharacter;
    const out = backfillPlayer(alreadyRun);
    expect(out.dog?.status).toBe('dead');
  });

  it('leaves a healthy dog untouched (no revive needed)', () => {
    const healthy = { ...base, dog: { ...deadDog(), status: 'with_player' as const, hp: 24 }, dogRevivedOta938: undefined } as PlayerCharacter;
