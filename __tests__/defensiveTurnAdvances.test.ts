...
describe('defensive verbs advance the round', () => {
  ...
  it('block triggers the enemy counter-attack (and the block-roll engages)', async () => {
    const store = useGameStore;
    await store.getState().hydrate();
    const race = getRaces()[0]!;
    const fac = getFactions()[0]!;
    ...
    store.getState().skipTutorial?.();
    // Ensure the player has an equipped weapon with defense > 0.
    ...
    store.setState({
      player: {
        ...
        inventory: [
          ...
          { id: 'main_blade', name: 'Rusted Blade', kind: 'weapon', quantity: 1, tags: ['weapon', 'blade', 'melee'] },
        ],
    ...
    expect(store.getState().player!.statusEffects?.some((e) => e.kind === 'blocking')).toBe(true);
