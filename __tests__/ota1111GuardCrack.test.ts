// OTA-1111 — guard-crack. Workstream A ("fights that end"): the owner's device
...
describe('OTA-1111 — three resisted hits wear the guard through', () => {
  ...
  beforeAll(async () => {
    ...
    if (!base) throw new Error('ota1111: Gutter Rat missing from enemies.json — roster drift');
    const foe: Enemy = {
    ...
    if (!scene) throw new Error('ota1111: no scene after startNewGame');
    store.setState({
      currentScene: {
        ...scene,
        enemies: [foe],
  ...
  function attackUntilResisted(n: number): void {
    ...
    if (resistedLines() < n) {
      throw new Error(`ota1111: only ${resistedLines()} resisted hits after ${safety} attacks`);
    }
  }
...
describe('OTA-1111 — source locks (damage floor + crack wiring)', () => {
