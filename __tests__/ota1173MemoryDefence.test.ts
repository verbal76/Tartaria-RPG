...
//
// ⚠ AND THE PREVIOUS OTA'S HOLD IS OVER. OTA-1172 deliberately shipped instruments only,
...
describe('OTA-1173 — a memory warning is ANSWERED, not just written down', () => {
  it('⚠⚠ OTA-1172 LOGGED IT AND DID NOTHING — that was only half a fix', () => {
  ...
  it('the release is reported, so a log shows the app defended itself', () => {
    // ⚠⚠ RETARGETED BY OTA-1179, AND THE NEW CLAIM IS STRONGER. This pinned the literal
...
describe('OTA-1173 — an iOS twitch no longer buys a 400MB reload', () => {
  it('⚠⚠ THE HELD DEFECT IS FIXED — `inactive` alone is not a return to the app', () => {
    ...
    // ⚠ OTA-1278 widened this handler (it now also restarts the foreground-settle
    // clock, so the watchdog cannot bypass App.tsx's re-warm debounce). The RULE
    // pinned here is unchanged: only a genuine `background` sets the flag.
...
describe('OTA-1173 — the reloads have a ceiling now', () => {
...
describe('OTA-1173 — what this OTA does NOT claim', () => {
  it('⚠⚠ THE CAUSE IS A HYPOTHESIS, AND THE FILE SAYS SO RATHER THAN ASSERTING IT', () => {
    ...
    // shipped in OTA-1172 and the reporting device was still on 1194. Every fix here is
