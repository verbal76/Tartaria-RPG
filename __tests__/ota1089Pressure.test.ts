...
// OTA-1136 — PRESSURE_PROFILES is keyed by PRESET now ('custom' composes from
...
describe('OTA-1089 — in the real store', () => {
  ...
  it('the toggle is the LAST step of creation, after the motive', () => {
    ...
    // RETARGETED BY OTA-1136: the call gained the optional CUSTOM payload
  ...
  it('the tide announcement fires once per stage and never re-fires', () => {
    ...
    // assertion broke when OTA-1090 landed a helper between them — a failure
    ...
    // OTA-1090 gave it a return value (did it speak?) so the Arbiter beat can
