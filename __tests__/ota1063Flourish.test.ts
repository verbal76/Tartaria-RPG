...
/**
 * OTA-1063 — THE FLOURISH. The last item on the owner's seven-item list, and
 ...
 * sentence. OTA-1058 shipped the conversation entirely synchronous because a
...
describe('OTA-1063 — THE DESIGN CONSTRAINT, carried forward', () => {
  // eslint-disable-next-line @typescript-eslint/no-require-imports
...
describe('OTA-1063 — the authored pool is the product', () => {
  ...
  it('the procedural cast is bucketed off the class key, with no role at all', () => {
    // OTA-1062 gave wanderers, roadside traders, escorts and Guardians their
  ...
  it('a wronged shopkeeper watches your hands', () => {
    // The one relationship state where the beat is doing real work: OTA-1058
...
describe('OTA-1063 — deterministic, and not a dice roll', () => {
  it('the same person, topic and state gives the same beat every time', () => {
    // Same reason as OTA-1049: somebody whose gestures reshuffle on a replay of
...
describe('OTA-1063 — the judge for a model line', () => {
  ...
  it('rejects the two registers this model actually falls into', () => {
    // OTA-1031: a second-person ACTION opener is scene text in the wrong slot.
  ...
  it('rejects an instruction echo', () => {
    // OTA-1030 caught the brief itself streaming raw to the screen.
...
describe('OTA-1063 — in the exchange', () => {
  ...
  const openTalk = () => {
    ...
    useGameStore.setState({
      pendingTalk: {
        ...
        // post-answer refresh rebuilds the same gate context it opened with.
        npcFaction: null,
