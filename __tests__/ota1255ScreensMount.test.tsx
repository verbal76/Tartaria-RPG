...
//
// Owner, on the coverage gap: *"fix these issues on golem only."*
//
// ⚠⚠ THIS IS THE CRASH CLASS I ALREADY SHIPPED ONCE. OTA-1245 hoisted a `useMemo`
// 82 lines above a helper it called; a memo FACTORY RUNS DURING RENDER, so under
// Hermes the const was in its temporal dead zone and read as `undefined`. The app
// died before a frame drew, on the owner's phone, on a build I had called green:
//
...
// `eslint`'s `no-use-before-define` WOULD, but measured, enabling it flags **2,869
// pre-existing sites**, so it cannot be a blocking gate. And every suite touching a
// screen SOURCE-PINS it. **Not one of them had ever mounted one.**
//
// OTA-1246 added that guard for ExplorationScreen alone. Measured before writing
// this: of the six main screens, ExplorationScreen had 8 suites that mount it and
// the other five had ZERO — InventoryScreen appeared in exactly one suite, as a
// `readFileSync` source pin.
//
// ⚠⚠ AN EMPTY RENDER IS NOT ENOUGH, AND THAT IS THE LESSON THAT COST THE MOST. The
// first version of OTA-1246's guard PASSED WITH THE BUG STILL IN: with no scene,
// the chip list mapped over an empty array and the dead reference was never CALLED.
// Every screen here is mounted against a real `startNewGame` player carrying real
// gear, a real scene, and a real vendor — so the loops that would touch a
// dead reference actually run.
...
describe('OTA-1255 — every main screen actually mounts', () => {
