...
describe('applyLoreLexicon', () => {
  it('respells Aetheric (case-insensitive)', () => {
    expect(applyLoreLexicon('Aetheric Spear')).toBe('eatheric Spear');
    expect(applyLoreLexicon('aetheric')).toBe('eatheric');
    expect(applyLoreLexicon('AETHERIC')).toBe('eatheric');
  });

  it('respells Aetherstone / Aether / Aetherborn distinctly', () => {
    expect(applyLoreLexicon('the Aetherstone Flood')).toBe('the eatherstone Flood');
    // Verify Aetherstone matches before plain Aether (longest first).
    expect(applyLoreLexicon('Aether vs Aetherstone')).toBe('eather vs eatherstone');
    expect(applyLoreLexicon('an Aetherborn warrior')).toBe('an eatherborn warrior');
  });

  it('respells Tartaria / Tartarian / Tartarians', () => {
    expect(applyLoreLexicon('walk into Tartaria')).toBe('walk into tar-tair-ee-uh');
    expect(applyLoreLexicon('a Tartarian Giant')).toBe('a tar-tair-ee-an Giant');
    expect(applyLoreLexicon('Tartarians remember')).toBe('tar-tair-ee-anz remember');
  });
