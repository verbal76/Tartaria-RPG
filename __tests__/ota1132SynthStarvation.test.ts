// OTA-1132 — ⚠ THE AMBIENT TRIM LANDED AND THE LINE STILL ARRIVED LATER.
//
// OTA-1131's own check, from the first device log running it:
//
...
// 31 tokens, which is ~2.5× the ~40ms/token it managed in the OTA-1130 log.
...
describe('OTA-1132 — a failed synthesis names its own culprit', () => {
  beforeEach(() => {
    resetQwenTelemetry();
    _resetCacheForTests();
  });
  ...
  it('⚠ an unparseable reply is reported WITH the text, not just a verdict', async () => {
    ...
    // The two things the OTA-1131 log could not tell us: how long it was, and
...
describe('OTA-1132 — the brief stops asking for an essay', () => {
...
describe('OTA-1132 — item synthesis yields to the jobs the player is waiting on', () => {
