// OTA-1121 — THE CACHED PREFIX. The first real win of the headroom track, and
...
// "above"/"below" pointers the move forces — OTA-1108 is the standing lesson
...
describe('OTA-1121 — the reusable prefix is large, and that is the whole point', () => {
...
describe('OTA-1121 — the ordering invariant itself', () => {
...
describe('OTA-1121 — ⚠ the prompt still says what it said, and points the right way', () => {
  it('ambient still omits every scene-reaction field (OTA-1106 holds)', () => {
    const p = prompt({ ambient: true });
  ...
  it('⚠ no rule points at a section that is no longer where it says', () => {
    // OTA-1108 lost a whole generation to exactly this: the ambient prompt
  ...
  it('ambient never regains the contradiction OTA-1108 removed', () => {
    const p = prompt({ ambient: true });
