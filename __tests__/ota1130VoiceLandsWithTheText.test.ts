// OTA-1130 — THE VOICE STOPS ARRIVING AFTER YOU HAVE ALREADY READ THE LINE.
//
...
// next. OTA-1128 measured what that is: a scene_intro generation runs 19.3
...
//
// ⚠ THIRD PIECE: PRE-SYNTHESIS. OTA-1129 banked scene intros so the TEXT lands
...
describe('OTA-1130 — ⚠ the voice now outranks the LLM', () => {
  it('the constants say so', () => {
    expect(ML_PRIORITY_VOICE).toBeGreaterThan(ML_PRIORITY_LLM);
    // …and homework is still below both, which OTA-1123 owns and this must
...
describe('OTA-1130 — ⚠ a line you already read past is not spoken', () => {
...
describe('OTA-1130 — ⚠ pre-synthesis: text and voice land together', () => {
  ...
  it('⚠ pre-synthesis runs at HOMEWORK priority — it must never delay a real line', () => {
    ...
    // RETARGETED BY OTA-1140 — the eviction comment block sits above the lock
  ...
  it('speak() consumes the cache at enqueue, and it is one-shot', () => {
    // ⚠ RETARGETED for OTA-1144: the take is now bound to `banked` first, because
  ...
  it('the cache is bounded — PCM is bulky and this is a latency buffer', () => {
    ...
    // RETARGETED BY OTA-1140 (pressure test) — the cap used to REFUSE when
...
describe('OTA-1130 — the answers to the other two questions, recorded', () => {
