// OTA-1178 — TWO THINGS THE OWNER HIT ON THE PHONE, BOTH OF THEM OURS.
//
...
//
//    OTA-1177 moved the recipient from `pendingGift` onto `giftMode` and sent the
...
describe('OTA-1178 #2 — the game stops refusing an action its own prose invited', () => {
...
describe('OTA-1178 #2b — the wait guard asks the verb table instead of remembering it', () => {
  ...
  it('⚠ an honest wait repair of a word the old list forgot is no longer thrown away', () => {
    // Both share a word with what the player typed, so the OTA-1016 invention
  ...
  it('and a genuinely invented wait is STILL rejected', () => {
    // The OTA-993 case this guard was built for: "clomb into the warp" coming back
...
describe('OTA-1178 #1 — a GIVE always puts you back where you can read the reply', () => {
  ...
  it('⚠ the old cleanup that cleared the WRONG field is gone', () => {
    // `set({ pendingGift: null })` on its own was OTA-1177's leftover: by then the
...
describe('OTA-1178 #3 — a dressed faction fighter is rank and file, never a boss', () => {
...
describe('OTA-1178 #4 — a container is not a prop in the room', () => {
  const SNIP = (() => {
...
describe('OTA-1178 #6 — a gift never claims standing it did not grant', () => {
...
describe('OTA-1178 #5 — the narrator may say the name of the room it is standing in', () => {
