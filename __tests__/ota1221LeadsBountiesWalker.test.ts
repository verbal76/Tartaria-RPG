...
// ⚠⚠ OTA-1221 — TEXAS RANGER RIDES THE LAST TWO FAMILIES: LEADS + BOUNTIES.
//
...
// the ota1214 audit; this walker additionally proves the trigger DOOR WORKS
...
// stamps → arrival on the contract's ground seeds the quarry (OTA-1166
...
describe('OTA-1221 — Texas Ranger on leads and bounties', () => {
  beforeAll(async () => {
  ...
  it('⚠⚠ bounty: the full chain, accept to on-the-spot payout', async () => {
    ...
    // ⚠ OTA-1165 — accepting REQUIRES the politics board frozen first (the
    ...
    expect(accepted?.politics).toBeTruthy(); // OTA-1165 freeze

    // Walk onto the contract's ground and take any action → quarry seeds (OTA-1166).
    ...
    // flagged groups exist and survive the maintenance cull (OTA-1221 exempts
