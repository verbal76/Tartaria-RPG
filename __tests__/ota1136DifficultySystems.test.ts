// OTA-1136 — DIFFICULTY THAT MEANS SOMETHING, AND A CUSTOM PICKER.
//
...
// OTA-1111 and OTA-1112 were written to fix).
...
describe('OTA-1136 — ⚠ the identity row protects the tuning', () => {
  it('⚠ EVERY dial is a no-op at the default tier', () => {
    ...
    // RETARGETED BY OTA-1140: the `hunger` dial is gone. It was written to
...
describe('OTA-1136 — pack size and the swing cap move together', () => {
  it('⚠ growing the pack grows the cap — otherwise it is longer, not harder', () => {
    // OTA-1112 capped melee swings per round because a five-raider wall was
...
describe('OTA-1136 — CUSTOM: pick the intensity, then pick what it touches', () => {
  ...
  it('⚠ custom with NOTHING checked is exactly the default run', () => {
    ...
    // RETARGETED BY OTA-1140: 'hunger' dropped — the dial was removed because
...
describe('OTA-1136 — ⚠ custom is not a back door up the ladder', () => {
...
describe('OTA-1136 — the dials reach the game', () => {
  const store = src('app/state/gameStore.ts');
