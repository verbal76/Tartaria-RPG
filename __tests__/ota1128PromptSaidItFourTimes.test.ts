...
// it — OTA-1031's third-person recap, the "Borderlands" playtest failure, the
...
// exactly what OTA-1122's bank did for ambient. That is a design call and it
...
describe('OTA-1128 — the rule is stated ONCE, not four times', () => {
  ...
  it('the ambient branch got the same dedup — its two copies also collapsed', () => {
    ...
    // Ambient still must not read the verb catalog — OTA-1106/1131's finding,
    // unchanged by this one.
    expect(p).not.toContain('brawl');
  });
});

describe('OTA-1128 — the arithmetic, pinned so it cannot drift back', () => {
  ...
  it('⚠ the peaceful prompt got materially smaller and must stay so', () => {
    ...
    // rather than duplication. OTA-1121 wrote this guard; OTA-1128 kept it
...
describe('OTA-1128 — ⚠ the combat branch is unreachable, and that is recorded', () => {
