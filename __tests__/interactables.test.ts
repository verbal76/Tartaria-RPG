...
import worldLadderData from '../app/data/world/worldLadder.json';
...
const microMicros: MicroMicro[] = (worldLadderData as {
  macroLocations: Array<{ microLocations: Array<{ microMicroLocations: MicroMicro[] }> }>;
}).macroLocations.flatMap((m) =>
  m.microLocations.flatMap((mi) => mi.microMicroLocations),
);
...
describe('authored interactables — Micro-Micro rooms (worldLadder)', () => {
  it('every Micro-Micro room declares an interactables array', () => {
    const missing = microMicros.filter(
      (r) => !r.interactables || r.interactables.length === 0,
    );
    if (missing.length > 0) {
      throw new Error(
        'Micro-Micro rooms missing interactables: ' +
        missing.map((r) => r.id).join(', '),
      );
    }
  });
  ...
  it('interactables are unique within each Micro-Micro room', () => {
    for (const room of microMicros) {
      const set = new Set(room.interactables);
      expect(set.size).toBe(room.interactables?.length ?? 0);
    }
...
describe('coverage — every location + hub room + Micro-Micro has at least 4 interactables', () => {
  ...
  it('Micro-Micro rooms meet the floor', () => {
    for (const room of microMicros) {
      expect(room.interactables?.length ?? 0).toBeGreaterThanOrEqual(4);
    }
  });
