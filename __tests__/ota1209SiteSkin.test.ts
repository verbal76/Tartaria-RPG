// OTA-1209 — A FACTION'S SITE WEARS ITS OWN COLOURS, NOT THE VISITOR'S.
//
...
describe('OTA-1209 — who owns a hub site', () => {
...
describe('⚠⚠ OTA-1209 — nine sites move, and NOTHING else does', () => {
...
describe('⚠⚠ OTA-1209 — the LAYOUT is untouched. Only names change.', () => {
  ...
  test('⚠ the broker is still at the gate under every faction’s skin', () => {
    // OTA-1208's entire fix rests on this. If a skin could move an anchor, the trading
...
describe('⚠⚠ OTA-1209 — no call site still passes the player’s faction raw', () => {
    ...
    for (const rel of ['app/state/gameStore.ts', 'app/components/InputBox.tsx', 'app/screens/MapScreen.tsx']) {
      ...
      for (const fn of ['hubRoomFor(', 'hubNameForFaction(']) {
        for (let i = src.indexOf(fn); i !== -1; i = src.indexOf(fn, i + 1)) {
          ...
          //
          // ⚠⚠ OTA-1282 (same fix as golem's ota1186 twin) — THIS USED TO ACCEPT THE
          // LITERAL NAME `skinFactionId`, WHICH GUARDED THE SPELLING RATHER THAN THE
          // RULE. A correct hoist under any other name failed (the gameStore's `skin`),
          // and a RAW faction id would have sailed through under the blessed name. A
          // bare identifier now counts only when this file actually assigns it from
          // the resolver, which is what the rule was ever about.
