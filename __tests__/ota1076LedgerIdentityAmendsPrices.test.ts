/**
 * OTA-1076 — the last of the Phase 1 residuals.
 *
 ...
 * strangers. Two consequences, both introduced by OTA-1072/1073:
 ...
 * OTA-1072 made it permanent and I flagged it as the owner's call. Permanent is
...
// OTA-1078 — the REAL function, not a copy of it. This suite used to
...
// OTA-1078 — a member of the real CAST. 'Road Hawker' is an archetype label,
// not a person, and `roadside:road_hawker` is now a LEGACY key the save-healing
// pass deletes on sight — see the OTA-1078 block at the bottom of this file.
const ROADSIDE_NAME = 'Grit Maalen';
const ROADSIDE_KEY = 'roadside:grit_maalen';

describe('OTA-1076 — one archetype is one person', () => {
  ...
  it('so meeting the fence three times actually makes them know you', () => {
    ...
    for (const [i, ms] of stamps.entries()) {
      ...
      m = rememberNpcMeeting(m, { id: ledgerId(spawn), name: spawn.name, role: spawn.title },
        // OTA-1078 — one in-game hour apart. Three spawns at a frozen clock are
  ...
  it('named and stall vendors keep their authored ids untouched', () => {
    ...
    // Hidden Market keeps its stall id AND gains the rep — see the OTA-1078
...
describe('OTA-1076 — the rows already leaked into live saves are swept up', () => {
...
describe('OTA-1076 — a caught theft can be paid off', () => {
  ...
  it('THE RESIDUE EXPLOIT: change from a settled debt does NOT pre-pay the next one', () => {
    // Shipped in OTA-1076 and caught by a test-quality audit. Settling a 600
...
describe('OTA-1076 — the relationship reaches the counter', () => {
...
describe('OTA-1078 — a Hidden Market stall is SIX people, not one', () => {
  ...
  // recordNpcSighting refreshes factionId from whoever is on shift, OTA-1077
...
describe('OTA-1078 — a fresh betrayal costs you your restitution', () => {
...
describe('OTA-1078 — saves written by 4.28.87/88 heal themselves', () => {
