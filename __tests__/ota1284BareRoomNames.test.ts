...
// ⚠⚠ OTA-1284 — PORT OF GOLEM OTA-1274's BARE-NAME INTERCEPT (+ its 1279
// widening). Owner, on hearing this hole was flagged for later rather than
// ported: "I don't want open errors." So it ports now.
//
// The original audit (golem OTA-1274) — "THERE WAS A ROOM CALLED BREAK":
//
...
describe('OTA-1284 — the names themselves', () => {
...
describe('OTA-1284 — typing a room name walks you there', () => {
  ...
  // ⚠⚠ Golem OTA-1279 replaced the original test's rule before this port. It used to pin earned fast-travel:
  ...
  it('⚠ the intercept never fires outside a hub', async () => {
    ...
    // ⚠ Fresh character, leaving from the SAFE gate: the walking tests above
    // can leave a spawned hostile in the deep rooms, and with an enemy in the
    // scene 'leave outpost' is combat movement, not a door. (Found live: an
    // Aetherborn Scavenger turned this test's leave into 'closing the gap'.)
