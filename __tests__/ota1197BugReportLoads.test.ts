// OTA-1197 — THE UPDATE PATH SAYS WHAT IT DID, AND THE BUG REPORT IS ACTUALLY LOADED.
//
...
//
// ⚠⚠ THAT INSTINCT DESERVED A TEST, NOT AN ASSURANCE. OTA-1194 landed on his device;
// OTA-1195 and OTA-1196 did not. The publish side was verified clean — `Channel 'hal2001'
...
// downloaded".
//
// ⚠ AND OTA-1195 ADDED THE MOST LOAD-BEARING IMPORT IN THIS CODEBASE TO A LEAF MODULE:
...
//
// ⚠⚠ NOTHING TESTED IT. `ota1195RuntimePressure` asserts on `aboutSummary.ts` as TEXT
...
describe('OTA-1197 — the boot update check reports itself to the DEVICE log', () => {
  it('⚠⚠ EVERY FAILURE USED TO GO TO console.warn, WHICH NO BUG REPORT CARRIES', () => {
    // The owner sat on OTA-1194 for hours while 1195 and 1196 were provably published to
...
describe('OTA-1197 — the bug-report module loads and runs', () => {
  it('⚠⚠ IMPORTING aboutSummary DOES NOT THROW — the OTA-1195 import chain executes', () => {
  ...
  it('the OTA-1195 runtime-pressure block is present in the built report', () => {
    const { buildBasicDeviceSummary } = require('../app/diagnostics/aboutSummary') as {
      buildBasicDeviceSummary: () => string;
    };
