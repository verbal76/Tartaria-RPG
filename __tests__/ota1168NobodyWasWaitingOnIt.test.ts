// OTA-1168 — NOBODY WAS WAITING ON IT.
//
// Owner, rejecting OTA-1167's shape of fix: *"we closed it yesterday, I even
...
//
// ⚠ HE IS RIGHT, AND THE DIFF SAYS SO. Between OTA-1157 (the commit he tested
...
// (160→280 ms), the presynth eviction repair — and OTA-1167's RESERVATION,
...
describe('OTA-1168 — the load no longer fires an interactive generation', () => {
  ...
  it('the existing valves are untouched — dedupe, readiness, and the gap', () => {
    // OTA-1132's one-at-a-time + 20 s gap is what keeps a salvage haul from
...
describe('OTA-1168 — the reservation shrinks to a guard rail', () => {
  ...
  it('the reversal is recorded with the owner\'s objection, not silently', () => {
    ...
    expect(LOCK).toContain('OTA-1168');
  });
});

describe('OTA-1168 — the homework slot is unchanged', () => {
