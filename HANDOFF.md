...
history in `HANDOFF-ARCHIVE.md`). Latest per line: **HaL2001 `2026-07-15-832`**,
**golem-line `…-812`**, **engine_Dev `…-1114`** (both 830 AND 831 are HAL+golem-only —
...
|---|---|---|---|
| 832 | 812 | ? | FUSED ARMOR NOUN NOW MATCHES ITS SLOT (player: "one of the feet slot armor is a girdle?"). `synthesizeFusionDeterministic` (itemFusion.ts) picked the armor NOUN from a flat pool (`Girdle/Harness/Plating/Cuirass/…`) while the SLOT was chosen by a separate rotation, so a waist word ("Girdle") or a torso word ("Harness") could land on the FEET slot. Fix: compute the forged slot FIRST (same hash-seeded rotation + recent-slot avoidance), then draw the noun from a slot-keyed pool — head → Helm/Crown/Hood/Visor/Coif/Cowl/Casque/Circlet; chest → Plate/Cuirass/Mantle/Carapace/Bastion/Aegis/Harness/Bulwark; legs → Girdle/Greaves/Faulds/Kilt/Legguards/Tassets/Brace; feet → Boots/Sabatons/Treads/Stompers/Warboots/Footguards/Striders. Forward-only (existing fused items keep their baked names). Lore-neutral code → HAL+golem (engine's synth is content-pack-driven; check separately). Tests: ota832FusedArmorSlotNoun (over 60 forges spanning every slot, the name's last word is always in its slot's pool; a feet piece is never a Girdle/Harness). |
