...
> **Latest OTA:** `2026-05-28-130` (Hook-puzzle UX polish — examine-the-noun peek surfaces current attempt state mid-puzzle; "rotate left" without a noun auto-routes when exactly one active puzzle hook accepts the intent; save/load round-trip preserves puzzleProgress; new tutorial step "When the narrative gives you a sequence" covers verbs, mercy threshold, and the deliberate decision to gate only the two puzzle-equipped hooks. 34/34 resolver tests pass. OTA-131 next: pressure test with strict OOM guardrails). See **Section 0** for the live issue tracker covering OTA-070 → OTA-130.
> **Recent session arcs:**
