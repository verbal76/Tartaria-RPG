...
- **[MECHANIC-1b]** STILL OPEN. Golem follower behavior — split out of MECHANIC-1 (which shipped the DC-fairness piece only). User asks: when a golem is summoned, can we send it to fight for the player; if it survives the combat, does it follow the player around until the next combat fires when it re-engages to fight again. Requires: (a) new follower-state slot on currentScene / player for the summoned golem instance; (b) a 'send golem' / 'command golem' combat verb in the combat verb set; (c) combat hook to auto-add the golem to enemies-side allies (or a separate ally pool) on the next combat trigger; (d) persistence across cardinal steps so it actually follows; (e) decay rule (golem expires after X hours? or after Y combats? — user hasn't specified, ask before implementing). Touches gameStore.ts combat + travel paths + types.ts CurrentScene shape.
