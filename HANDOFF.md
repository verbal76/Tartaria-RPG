...
**This checkout:** branch `golem-line` — **HAL's warm standby** — kept current with HaL2001 (minor changes ship to both in one pass); the fork point for future engine-breaking work so production is never at risk. Same game, channel `golem-line`, installs side-by-side with HAL.
