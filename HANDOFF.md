...
#### OTA-288 (Mire Coil) — Qwen threads 4→2 + RE-DOWNLOAD QWEN MODEL button (Pixel 10 testbed)

- **OTA-288 · Player on Pixel 10 Pro XL (at 1/2 mid-use crash count after OTA-287's mid-use detector): *"ship A and B. take my phone off of the block list and will use mine as the tests run."*** Two cheap mitigations before committing to the AAB rebuild path. Player's Pixel 10 Pro XL stays explicitly OFF any block list — production testbed for whether either of these helps.
- **A. Thread count default 4 → 2** in `QwenGenerativeEngine.initialize`. From the upstream-fix discussion (Termux / NDK crash threads): "Llama.cpp sometimes crashes if you assign more physical cores than you have. Lower your thread count." Fewer concurrent ggml workers reduce simultaneous SIMD-memcpy pressure. Modest perf cost (~10-20% on mid-range mobile CPUs). **Honest probability:** probably won't fix the SVE-on-Tensor-G4 signature (the crash is at SVE instruction execution, not race condition) — but it's free to try and may help latent races we don't have signatures for.
- **B. NEW "RE-DOWNLOAD QWEN MODEL" button** in About → Voice → Bundled (sage outline, parallel to the existing CLEAR BUNDLED VOICE CACHE button for Kokoro). Calls new `ModelDownloader.clearQwenCache()` → nukes the cached GGUF → `shutdownQwen()` + `bootQwen()` to force a fresh download. **Honest probability:** if a transient network truncation passed the >200 MB size check but left byte errors, every generate fires on the same bad region — this fixes it. If it's the SVE compile-flag issue, redownload changes nothing (the same file would still hit the same instruction).
- **The real fix remains the AAB rebuild** per the upstream confirmation: `-DGGML_SVE=OFF` at compile time, which our existing patch-package blocklist achieves by forcing the `v8_4_fp16_dotprod` variant (compiled without `+sve`). Mire Coil tests the cheaper hypotheses first; if Pixel 10 Pro XL keeps crashing, the AAB rebuild becomes the next move. Testbed answers the question cleanly.
- **Cross-platform** OTA, no marker. iPhone testers benefit minimally (lower battery drain on background narration; the cache button is iOS-callable too).
- **Files:** `app/ai/generation/QwenGenerativeEngine.ts` (threads default 4 → 2 in `initialize` + JSDoc updated), `app/ai/ota/ModelDownloader.ts` (new `clearQwenCache` method + JSDoc), `app/screens/AboutScreen.tsx` (ModelDownloader import + `handleClearQwenCache` handler + `qwenCacheCleared` state + RE-DOWNLOAD QWEN MODEL button below the bundled-voice section), `app/buildCodename.ts` (Mire Coil added), `app/buildInfo.ts` (OTA-288 bump + change note), `docs/build-codenames.md` (Mire Coil moved to current; pool renumbered), HANDOFF.md (this entry).
