...
> **Latest OTA:** `2026-05-28-154` (SUMMON chip on the home-screen MAIN QUEST card — OTA-148 put SUMMON on the Contracts PRIMARY OBJECTIVE card and OTA-149 added the `summon guardian` verb path, but the Contracts surface is one level deep. Player: "I want to be able to get right to the city smack that button and have at it." MAIN QUEST chip on ExplorationScreen restructured to a row layout — existing title + subtitle on the left, ★ SUMMON button on the right when player is in an unrecovered Lost Capital with mainQuest in revelation/cores. Same `atUnrecovered` precondition the chip's hint line already uses. Nested TouchableOpacity captures the tap so smacking SUMMON doesn't also fire the chip's tap-to-Contracts navigation. Contracts SUMMON chip from OTA-148 stays in place as a backup path.). See **Section 0** for the closed-issue archive.
> **Recent session arcs:**
