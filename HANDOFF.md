...
- **OTA-561 "Unpenthexium Forbearance" — [bugfix] the 559/560 guard fixes silently no-op'd on-device, and threshold 1 was too trigger-happy** *(element #156)*. A post-560 log showed Qwen STILL auto-disabled with the retry schedule intact (the OTA-560 amnesty never ran) and the voice count unchanged. Cause: both new functions used `AsyncStorage.multiRemove`, which silently no-ops on the device's AsyncStorage build (jest mocks implement it, so it passed tests) — so neither the breadcrumb-clear-on-background nor the amnesty cleared anything. Fix: (1) `clearInFlightBreadcrumbs` + `healStaleGuardState` now use `removeItem` via `Promise.all` (the pattern `resetMLHealth` already uses); amnesty version bumped arb126→arb127 so the now-working amnesty re-runs. (2) `MAX_QWEN_COMPLETION_CRASHES` restored 1→3 — the breadcrumb can't tell a real SVE SIGSEGV from a benign mid-generation app close, so threshold 1 re-benched a device that never crashes; with OTA-559's success-reset only CONSECUTIVE failures accumulate, so a broken device still trips after 3 while a healthy one oscillates 0↔1. `app/diagnostics/mlHealth.ts`. Tests: false-disable mock now OMITS multiRemove (regression guard); 4 ML guard suites / 23 tests green.

- **OTA-560 "Unpentpentium Reprieve" — [bugfix] immediately un-bench an Arbiter the pre-559 detector falsely disabled** *(element #155)*. OTA-559 stops FUTURE false trips, but a device already benched (Qwen disabled + a phantom Kokoro voice crash from one benign close) would otherwise wait out the auto-retry cooldown. New `healStaleGuardState()` runs once per install at boot, BEFORE `loadMLHealth`, and forgives the completion/voice guard history accrued by the pre-fix detector: clears the Qwen completion crash count + disable + retry schedule and the voice crash count, pinned by a guard-version key so it never repeats. General init guard (OTA-558) + boot counter untouched; a genuinely-bad device re-trips at its next REAL foreground crash. The falsely-disabled Arbiter returns on the very next boot. `app/diagnostics/mlHealth.ts`, `App.tsx`. Tests: 2 new (10 in false-disable suite); 4 ML guard suites / 21 tests green.

- **OTA-559 "Unpentquadium Steadfast" — [bugfix] stop a benign app-close from re-benching the Arbiter** *(element #154)*. OTA-558 got Qwen to `qwen:done`, but the next log showed it auto-disabled AGAIN: "Qwen completion guard: auto-disabled after 1 completion crashes" + "VOICE CRASH detected — kokoro:af_heart", BOTH on the same launch on a device that never crashes (Kokoro audibly plays every session). The Qwen-completion + TTS breadcrumbs can't distinguish a real foreground SIGSEGV from a BENIGN exit (swipe-away / OS background / OTA reload mid-narration leaves the identical surviving breadcrumb). OTA-464 already made VOICE detection-only for this reason, but the Qwen completion guard still auto-disables at threshold 1 — so one benign close (which left BOTH breadcrumbs, same event double-counted) benched the Arbiter. Fix: the App.tsx AppState handler now clears the in-flight breadcrumbs on leaving the foreground (an ORDERLY exit — nothing crashed); a breadcrumb surviving to next boot then means a genuine FOREGROUND crash, the only real signal, and since generation/narration only run foregrounded this never masks a true failure. Plus a clean completion now wipes lingering completion-crash suspicion (symmetric self-heal). `app/diagnostics/mlHealth.ts`, `App.tsx`. Tests: 4 new (8 in false-disable suite) green.

- **OTA-558 "Unpenttrium Rekindle" — [bugfix] actually re-enable falsely-disabled Qwen** *(element #153)*. OTA-557's gate change wasn't reached. Two deeper faults: (1) the general crash detector never cleared KEY_ATTEMPTED after counting, so a stale 'attempted, no success' breadcrumb re-counted a PHANTOM crash every boot (74→78 just by launching) — now cleared after counting; (2) App.tsx gated BOTH classifier + Qwen on the general shouldAttemptMLInit, skipping everything before the OTA-557 shouldAttemptQwen bypass — now the classifier stays strict but Qwen (deferred + crash-caught) re-attempts via shouldAttemptQwen, and a successful init resets the general state to heal the classifier next launch. `app/diagnostics/mlHealth.ts`, `App.tsx`. Tests: 4 false-disable + 38 ML/Qwen green.
