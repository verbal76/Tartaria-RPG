...
history in `HANDOFF-ARCHIVE.md`). Latest per line: **HaL2001 `2026-07-16-837`**,
**golem-line `…-817`**, **engine_Dev `…-1118`** (836–838 are three Tier-1 QoL OTAs
— "make the depth visible." 836 = tap-to-explain AC; 837 = discovery codex/bestiary
+ surfacing the concepts lore bank. Both ship to all three lines: sheet/codex UI +
a display helper, no combat-math change. NOTE for engine: engine's LoreCodexBody
may pull races/factions/enemies/concepts from content packs (getRaces/getX) rather
than the JSON imports — reconcile the 837 imports on that line. 836 was pure sheet UI. 835 ships to ALL THREE — the engine
