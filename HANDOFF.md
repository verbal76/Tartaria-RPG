...
**This checkout:** branch `engine_Dev` — **the lore-agnostic RPG ENGINE line** — a separate, parallel product (content-pack driven; see §2). NOT the live Tartaria game: that is `HaL2001` (production, channel `hal2001`+`preview`+`ios-preview`), with `golem-line` as its warm standby. A push here OTAs only the engine channel.
