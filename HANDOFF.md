...
> **Latest OTA:** `2026-05-28-129` (Hook-puzzle foundation — the longest-open dead-hook issue closes. Engine narration that says "three rotations, in the right order" / "knock on the steeple" is now backed by real puzzle resolution. 7 new parser intents (rotate, knock, turn, twist, press, push, pull), new hookPuzzles.ts module with definitions for sealed_vault_door + submerged_steeple, hint copy at failure thresholds, mercy auto-solve to anti-stuck. 22/22 resolver tests pass. OTA-130 next for more puzzles + polish, OTA-131 for stress-tested ship). See **Section 0** for the live issue tracker covering OTA-070 → OTA-129.
> **Recent session arcs:**
