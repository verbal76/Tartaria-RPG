...
17. **Phase 4 §2.2 — Aliases for fuzzy target resolution.** ✅ Landed (commit b32dd4c). Aliases now live on Location + Enemy types; collectSceneNouns includes them; matchAmbientNoun case-sensitivity bug fixed; 21 locations + 19 enemies seeded.
...
20. **Phase 4 §4.3 — Biome-weighted encounter/loot distribution.** ✅ Landed (this commit). pickEncounterFromLadder and pickLootFromLadder rarity-weight inside the Micro-Micro's curated pools; beginScene wires the encounter path; loot path remains available for area-search / dig handlers when those want to consult the curated pool too (currently they use the existing area-search outcome system — wiring is a follow-up if desired).
