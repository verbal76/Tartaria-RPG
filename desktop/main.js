...
// desktop window for the RPG Engine (dev). Mirrors the live-Tartaria steam_Dev
// desktop wrapper, minus the Steam/store integration (this is an internal dev
// tool, not a store product). Kept ISOLATED: separate appId, separate log file,
// ENGINE_* env vars — it can't touch the mobile engine_Dev line.
//
...
//
// 2) Writes a DIAGNOSTIC LOG to the user's Desktop ("RPG-Engine-Dev-log.txt")
...
function resolveLogPath() {
  const candidates = [];
  try { candidates.push(path.join(app.getPath('desktop'), 'RPG-Engine-Dev-log.txt')); } catch {}
  try { candidates.push(path.join(app.getPath('userData'), 'RPG-Engine-Dev-log.txt')); } catch {}
  candidates.push(path.join(__dirname, 'RPG-Engine-Dev-log.txt'));
...
async function createWindow() {
  ...
  // DevTools off by default (open detached DevTools adds real overhead). Opt in
  // with ENGINE_DEVTOOLS=1 when you need the console.
  if (process.env.ENGINE_DEVTOOLS === '1') wc.openDevTools({ mode: 'detach' });

  // F11 fullscreen.
  wc.on('before-input-event', (_evt, input) => {
    if (input.type === 'keyDown' && input.key === 'F11') win.setFullScreen(!win.isFullScreen());
  });
  if (process.env.ENGINE_FULLSCREEN === '1') win.setFullScreen(true);

  // DEV: live Expo web server. PROD: serve the static export over localhost.
  const devUrl = process.env.ENGINE_DEV_URL;
  if (devUrl) {
...
// GPU instead of blocking the main thread with WASM. Must be set before app ready.
app.commandLine.appendSwitch('enable-unsafe-webgpu');

app.whenReady().then(() => {
  LOG_PATH = resolveLogPath();
  logLine('──────── RPG Engine (dev · PC) starting ────────');
