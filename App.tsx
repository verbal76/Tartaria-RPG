...
try {
  ...
  if (errorUtils?.setGlobalHandler) {
    ...
    errorUtils.setGlobalHandler((err, isFatal) => {
      ...
      // ⚠⚠ OTA-1371 — AND THE LEDGER, which is the durable copy. The
...
export default function App() {
  ...
  // ⚠ OTA-1354 — the classifier resume gets the same settled-foreground debounce
  ...
  useEffect(() => {
    ...
    // ⚠⚠ OTA-1371 — hydrate the crash ledger and the delivery preference at
  ...
  useEffect(() => {
    const onChange = (status: AppStateStatus) => {
      if (status === 'background' || status === 'inactive') {
        ...
        // OTA-1354 — a foreground visit shorter than the debounce never
        ...
        // OTA-1354 — debounced, mirroring the Qwen re-warm below. The classifier
    ...
    return () => {
      ...
      if (cognitiveResumeTimer.current) { clearTimeout(cognitiveResumeTimer.current); cognitiveResumeTimer.current = null; } // OTA-1354
    };
...
> {
  ...
  componentDidCatch(error: Error, errorInfo: { componentStack?: string }) {
    ...
    // ⚠ OTA-1371 — a recovered screen crash is still a crash, and it is the one
