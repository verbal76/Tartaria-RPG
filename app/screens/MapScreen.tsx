...
//
// ⚠ OTA-1338 SCRUB — this file used to carry the player-dot positioning chain
...
// ⚠ OTA-1346 — how long ⌖ ME takes to carry the view to the marker. Tuned SLOW on
...
export function MapScreen() {
  ...
  // ⚠⚠ OTA-1372 — WHAT A PINCH HAS TO REMEMBER, AND WHAT IT USED TO FORGET.
  ...
  // ⚠ OTA-1338 SCRUB — two blocks died here: the OTA 23-003/035 auto-focus-the-hub-inset
  ...
  // ⚠⚠ OTA-1344 — THE PLAYER MARKER RETURNS, AND THE REASON IT CAN. OTA-182 removed
  ...
  const centerOnPlayer = (frac: { fx: number; fy: number } | null) => {
    ...
    // ⚠ OTA-1346 — GLIDE, DON'T CUT. Owner: *"center and zoom in on it a little
  ...
  // ⚠ OTA-1355 — opening the map INSIDE an outpost glides to your room on its
  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (e, gestureState) => {
        ...
        if (ts.length >= 2 && startPinchDist.current > 0) {
          // ⚠⚠ OTA-1372 — PINCH ANCHORED ON THE FINGERS, NOT ON THE BOX CENTRE.
          //
  ...
  // OTA-1344 — the "you are here" marker: the current location's cell through the
  ...
  if (imgBox) {
    ...
    // ⚠⚠ OTA-1355 — THE MARKER WALKS THE OUTPOST TOO. Owner: *"when a player is
    ...
    // world atlas earned in OTA-1344 now stands on the CURRENT room of the
  ...
  // OTA-1344); it stays now the marker is back, because a sentence that names the
    ...
    <View style={styles.container}>
      ...
      <View
        ...
        onLayout={(e) => {
          ...
          // OTA-1372 — the pinch anchor needs the box's PAGE origin, because
        ...
        >
          ...
          {playerMarkerBox && (() => {
            ...
            const ring = {
              ...
              // OTA-1346 — thinner stroke by owner request ("thin the lines on the
              // icon"): half the OTA-1344 weight, floored where it stays visible.
...
const styles = StyleSheet.create({
  ...
  // OTA-1344 — the marker's two cross-fading liveries. Colors only — geometry is
  ...
  // OTA-1355 — the outpost map's explored-room ✓: the marker's green, the same
