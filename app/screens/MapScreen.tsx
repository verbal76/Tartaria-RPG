...
import { outpostRoomMark } from '../engine/outpostRoomMarks'; // OTA-1351 — the marker walks the outpost too
...
// ⚠ OTA-1342 — how long ⌖ ME takes to carry the view to the marker. Tuned SLOW on
...
/** ⚠ OTA-1363 — MOVED TO app/engine/mapFraction.ts, AND THAT MOVE IS THE POINT.
...
export function MapScreen() {
  ...
  // ⚠ OTA-1351 — rooms already walked, for the outpost map's ✓ marks. The SAME
  ...
  // ⚠⚠ OTA-1363 — WHAT A PINCH HAS TO REMEMBER, AND WHAT IT USED TO FORGET.
  ...
  const centerOnPlayer = (frac: { fx: number; fy: number } | null) => {
    ...
    // ⚠ OTA-1342 — GLIDE, DON'T CUT. Owner: *"center and zoom in on it a little
  ...
  // ⚠ OTA-1351 — opening the map INSIDE an outpost glides to your room on its
    ...
    PanResponder.create({
      ...
      onPanResponderMove: (e, gestureState) => {
        ...
        if (ts.length >= 2 && startPinchDist.current === 0) {
          ...
          startMidX.current = m.x;
          startMidY.current = m.y;
          // ⚠ OTA-1363 — rebase dx with the rest of the baseline. This line is
        ...
        if (ts.length >= 2 && startPinchDist.current > 0) {
          // ⚠⚠ OTA-1363 — PINCH ANCHORED ON THE FINGERS, NOT ON THE BOX CENTRE.
          //
          ...
          // Single-finger pan. ⚠ OTA-1363 — the delta is measured from the
  ...
  // OTA-1351 — ✓ marks for outpost rooms already walked (hubVisited), drawn on
  ...
  if (imgBox) {
    ...
    if (!showingOutpost) {
      ...
      // ⚠ OTA-1343 — anchored to the player's AUTHORITATIVE absolute cell
    ...
    // ⚠⚠ OTA-1351 — THE MARKER WALKS THE OUTPOST TOO. Owner: *"when a player is
    ...
    // world atlas earned in OTA-1340 now stands on the CURRENT room of the
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        {/* OTA-1340 — jump the view to the pulsing "you are here" marker.
            OTA-1351 — the outpost interior has a marker now too (the current
      ...
      <View
        ...
        onLayout={(e) => {
          ...
          // OTA-1363 — the pinch anchor needs the box's PAGE origin, because
      ...
      >
        ...
        >
          ...
          {/* ⚠ OTA-1351 — outpost rooms already walked wear a ✓ on the interior
          ...
          {playerMarkerBox && (() => {
            ...
            const ring = {
              ...
              // OTA-1342 — thinner stroke by owner request ("thin the lines on the
              // icon"): half the OTA-1340 weight, floored where it stays visible.
...
const styles = StyleSheet.create({
  ...
  // OTA-1351 — the outpost map's explored-room ✓: the marker's green, the same
