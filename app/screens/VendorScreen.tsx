...
export function VendorScreen() {
  ...
  const rapportMod = vendorPriceMod(
    ...
    vendor?.faction,
    // OTA-1337 — the standing ladder: same fourth argument the store passes, so
  ...
  // confirm, so the sweep inherited no such stop — one tap could silently sell the
  // last Hardened Climbing Strap the single-item path would have made you confirm
  // in red. Measured target: Aether-Breath Mask, a Common armor carrying gate
  // `breathe_toxic`, was in the sweep. A SPARE copy still sells (gateLossFor is
  // null when quantity > 1 or another gate-satisfier exists); only the last one is
  // held out.
  const bulkSellable = sellable.filter(({ item }) => !gateLossFor(item.name));
  // OTA-1345 — the hold-back is SAID: the confirm names each held piece and why,
  // computed from the same gate filter that does the holding so text and
    ...
    <View style={styles.container}>
      ...
      <View style={styles.vendorCard}>
        ...
        {/* OTA-805 — rapport price break. Shown once the player has earned dealing
            with this faction (rapport quest, Charisma) — and, OTA-1337, once the
