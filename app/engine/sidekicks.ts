...
//
// Each entry maps a SidekickKind to:
//   - display label
...
//
// To add a new golem type: add to SidekickKind in types.ts, append
// here, optionally surface in the recipe-list UI so the player
// can see what fuel each variant needs.

import type { Companion, SidekickKind, Rarity } from './types';
import type { SidekickStatKey } from './types';

export interface SidekickRecipeFuel {
...
export interface SidekickDefinition {
   ...
   *  immunity — see sidekickDamageResist + the retaliation min-1-damage rule). */
  resistBase: number;
  resistCap: number;
  /** engine_Dev — words the player can type after "summon" to call THIS sidekick
   *  (e.g. ['phase', 'phase automaton']). parseSidekickKind matches the longest alias
   ...
   *  SIDEKICK_ELEMENT_TAGS for uploaded summons). */
  elementTags?: string[];
}
...
export const SIDEKICK_DEFINITIONS: Record<SidekickKind, SidekickDefinition> =
  (builtinSidekickData as { definitions: Record<SidekickKind, SidekickDefinition> }).definitions;
...
function readSummonsOverride(): { noun?: string; defs: SidekickDefinition[] } | null {
  try {
...
export function resolveSidekickDefs(): SidekickDefinition[] {
  const ov = readSummonsOverride();
  if (ov && Array.isArray(ov.defs) && ov.defs.length > 0) return ov.defs;
  return Object.values(SIDEKICK_DEFINITIONS);
}
...
export function getSidekickDefinition(kind: SidekickKind): SidekickDefinition {
  const defs = resolveSidekickDefs();
...
export function isSidekickWeapon(weaponTags: readonly string[] | undefined): boolean {
...
export function parseSidekickKind(text: string): SidekickKind | null {
  const t = text.toLowerCase();
  const defs = resolveSidekickDefs();
  // Build (alias → kind) candidates, longest alias first so "iron golem" beats "iron".
  const cands: { needle: string; kind: SidekickKind }[] = [];
  for (const d of defs) {
...
export function makeCompanion(def: SidekickDefinition): Companion {
  return {
    kind: def.kind,
    name: def.name,
    hp: def.hpMax,
...
const SIDEKICK_LEVEL_UP_THRESHOLD = 100;

/** Read a trained golem stat, tolerating golems summoned before OTA-467. */
export function sidekickStatBonus(golem: Companion, key: SidekickStatKey): number {
...
export function sidekickDamageResist(golem: Companion): number {
...
export interface SidekickTrainResult {
  golem: Companion;
  leveled: { stat: SidekickStatKey; from: number; to: number } | null;
}
...
export function trainSidekickStat(
  golem: Companion,
  stat: SidekickStatKey,
  success: boolean,
): SidekickTrainResult {
  ...
  let next = baseStat;
  let leveled: SidekickTrainResult['leveled'] = null;
  while (progress >= SIDEKICK_LEVEL_UP_THRESHOLD) {
    progress -= SIDEKICK_LEVEL_UP_THRESHOLD;
    const before = next;
    next = before + 1;
  ...
  // sidekickDamageResist and the retaliation rule.)
...
export function sidekickRepairParts(kind: SidekickKind): string[] {
  return Array.from(new Set(getSidekickDefinition(kind).fuel.map((f) => f.name)));
}

/** OTA-466 — true if `itemName` is one of the parts this golem is made of
 *  (case-insensitive). Only constituent parts can repair it. */
export function isSidekickRepairPart(kind: SidekickKind, itemName: string): boolean {
  const lower = itemName.toLowerCase().trim();
  return sidekickRepairParts(kind).some((p) => p.toLowerCase() === lower);
}
...
export function sidekickRepairHeal(kind: SidekickKind): number {
  ...
  return Math.max(4, Math.round(getSidekickDefinition(kind).hpMax / 3));
}
 ...
 *  `elementTags`. Read via sidekickElementTags(kind), not this map directly. */
export const SIDEKICK_ELEMENT_TAGS: Record<string, readonly string[]> = {
  mud_golem: ['mud'],
...
export function sidekickElementTags(kind: SidekickKind): readonly string[] {
  const def = getSidekickDefinition(kind);
  if (def?.elementTags && def.elementTags.length > 0) return def.elementTags;
  return SIDEKICK_ELEMENT_TAGS[kind] ?? [];
}
 ...
 *  rate (see sidekickSubstituteHeal). Restricted to misc/material items so you
 *  can't feed it an aether-tagged WEAPON or armor piece. */
export function isSidekickSubstitutePart(
  kind: SidekickKind,
  item: { name: string; kind?: string; tags?: readonly string[] },
): boolean {
  if (item.kind && item.kind !== 'misc') return false; // materials/loot only
  if (isSidekickRepairPart(kind, item.name)) return false; // exact fuel → full-heal path
  const el = sidekickElementTags(kind);
 ...
 *  *something*. */
export function sidekickSubstituteHeal(kind: SidekickKind, rarity?: Rarity | null): number {
  const full = sidekickRepairHeal(kind);
