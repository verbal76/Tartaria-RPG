...
// and the noun the player named. The lines are deliberately lore-neutral
// (this is the engine template — narrative flavor is owned by the content
// pack): the action reaches out, and a long-abandoned place answers with
// silence, memory, or the sense of being the first to try in a long time.
...
// "the dark ahead" phrasing when they named nothing (handled by fillNoun).
const LINES: Record<GestureFamily, string[]> = {
  knock: [
    ...
    'You pound on {noun}. The echo runs off into the dark and does not return with company. This place keeps its doors, and its silences.',
  ],
  sound: [
    'You set {noun} sounding. The note hangs, thins, and is gone — a signal thrown out to a place that stopped listening a long time ago.',
    'You sound {noun}. For a breath the empty rooms seem to lean in to hear it. Then the heavy quiet folds back over everything.',
  ...
  touch: [
    ...
    'You run your fingers over {noun}. Grit, old varnish, the ghost of a carved line — a whole life pressed into a surface no one has touched in a very long time.',
    'You touch {noun}. Something almost stirs at the contact — not warmth, not sound, just the sense that this place noticed, and remembered being noticed.',
  ],
  reverent: [
    'You bow your head at {noun}. Nothing answers, but the stillness has a weight to it, and you leave a little of your unease behind.',
    'You pay {noun} its due. Whatever was honored here is long gone — yet the gesture steadies you, the way old gestures do.',
    'You kneel before {noun}. No blessing comes. The place simply lets you rest a moment in the shape of something older than you.',
  ],
  manipulate: [
    'You work {noun} with both hands. It shifts a hair, groans, and holds — set in place by long years and patience. But it moved, and it remembers you tried.',
    'You lean into {noun}. It resists the way everything here resists: not with malice, only with the sheer stubborn weight of time. Nothing gives, but something is noted.',
