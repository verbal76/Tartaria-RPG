...
/** ⚠⚠ OTA-1363 — THE POOL THE SPAWNER DRAWS FROM, local dead AND imported.
 *  The single join between the shared ledger and the existing Hollowed
 *  machinery: everything downstream — the scaling, the intro beats, the
 *  reclaim, the defeat lines — already works on a FallenHero and never asks
 *  where it came from. A foreign corpse is just one more name on the roll.
 *
 *  ⚠ NOT name-gated, deliberately. A locked player's ledger is empty, so this
 *  returns exactly what it returned before and costs nothing; gating it too
 *  would mean an unlocked player later finds the machinery in a state it was
 *  never tested in. The gate is on the EXCHANGE panel, which is the only way
 *  anything gets into the ledger in the first place.
 *
 *  Kept lazy-required so the pure revenant module never hard-depends on
 *  storage, and so a ledger that fails to load costs the extra pool and
 *  nothing else. */
...
/** ⚠⚠ OTA-1358 — THE CLONE'S OWN NUMBERS.
 *
...
export function revenantFromFallen(f: FallenHero, playerHpMax: number): Enemy {
  ...
  return {
    ...
    hp,
    rarity: 'Legendary',
    // ⚠⚠ OTA-1363 — THE GEAR FAUCET. A foreign corpse yields its WEAPON only:
    ...
    // where the volume lives — four slots per corpse times however many houses
    // — and that inflow comes from saves this player does not control. Rarity
    // is untouched on purpose: degrading a foreign Legendary would lie about
    // what that character actually carried. Your own dead are unchanged.
...
export function buildFallenGearSnapshot(p: {
  ...
  // ⚠ OTA-1358 — ten slots, not six. Rings and an amulet are part of the kit
