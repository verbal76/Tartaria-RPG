...
export interface Wanderer {
  ...
  faction: string | null;
  /** OTA-1094 — concrete goods on them (shaken loose by a successful INTIMIDATE). */
  goods: WandererGoods;
  /** OTA-1094 — a location lead they'll give up to a successful PERSUADE (paid out
...
// ── OTA-1094 (Phase 2) — PROCEDURAL PAYLOAD + DIALOGUE BEATS ────────────────────
...
// "cagey" beat — the pause before the parley where they name their price.
...
export function makeWandererGoods(seed: number, temperament: Temperament): WandererGoods {
  ...
  const tc = (rich ? 12 : 7) + (Math.abs(seed) % (rich ? 16 : 9));
...
export function makeWandererLead(seed: number): WandererLead {
  ...
  const rewardTc = 14 + (Math.abs(seed) % 22);
  const rewardItem = seededPick(GOODS_POOL, seed, 53);
  return { hint: `${cache}, off to the ${dir}`, rewardTc, rewardItem };
}
