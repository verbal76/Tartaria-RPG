...
// OTA-1004 — the catch-all pool is now fully POSTURE-NEUTRAL (ports Tartaria
...
// hands", "weigh", "let it go". (Lore-neutral for the content-pack engine.)
export const GENERIC_VARIANTS: readonly string[] = [
  'You look the {noun} over. Nothing about it sings, nothing about it warns — the world is full of things waiting to be remembered.',
  'You study the {noun} and find no answer. The silence here is patient; whatever it knew, it has decided to keep.',
  'The {noun} reads as ordinary, which out here is a small kind of relief. You leave it be.',
  'You take the measure of the {noun}. The haze does not gather; the dust does not stir. Just a thing, in a place.',
...
// OTA-1001 — fixed-feature generic pool. The GENERIC_VARIANTS above lean on
...
// instead. Lore-neutral (a content pack can override via 'investigateFixed').
export const FIXED_FEATURE_VARIANTS: readonly string[] = [
  'You look the {noun} over where it sits. Nothing about it sings, nothing about it warns — the world is full of things waiting to be remembered.',
  'You study the {noun} and find no answer. The silence here is patient; whatever it knew, it has decided to keep.',
  'The {noun} reads as ordinary, which out here is a small kind of relief. You leave it be.',
  'You run a hand along the {noun}. Cold, set fast in place, and about as talkative as the rest of this place.',
  'You give the {noun} a long look. It returns the gesture by being exactly what it appears to be — no more, no less.',
  'The {noun} resists your reading. Not hostile — just closed. Some things don\'t open for the curious.',
  'You study the {noun}. It bears no marks worth naming — no glyph, no fingerprint, no trace worth cataloguing.',
  'The {noun} keeps its silence, and it isn\'t going anywhere for you to press it. Nothing leans in. You move on.',
];
...
export function resolveLore(entry: InvestigationEntry): string {
  ...
  if (creepyPool && creepyPool.length > 0) {
    ...
    // OTA-1001 — fixed features (stair, wall, hatch …) draw from the
    ...
    // Both pools are content-pack overridable.
    const fixedPool = resolveFlavor('investigateFixed', FIXED_FEATURE_VARIANTS) as readonly string[];
    const pool = FIXED_FEATURE_RE.test(entry.noun.toLowerCase()) ? fixedPool : genericPool;
