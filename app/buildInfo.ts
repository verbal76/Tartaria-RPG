...
//          pool renumbered),
//          HANDOFF.md (closed issue entry).
// OTA-291 (Thorn Vault) — NATIVE BUILD (AAB + APK + OTA). The real
//   fix for the Pixel 10 Pro XL / Tensor G4 Qwen crash class, plus
//   the safe portions of the rolled-back Brass Helm + Mire Coil
//   changes. Builds both AAB (for Play Console upload) and APK (for
//   sideload test on the player's Pixel) per request:
//   "push all of that into an apk and an aab, I will test it on my
//   phone, if it works with no errors I will push the aab."
//
//   THREE COUPLED CHANGES:
//
//   1. PATCH-PACKAGE BLOCKLIST EXTENSION. Extends
//      isKnownMisclassifiedChip() in node_modules/llama.rn/.../
//      LlamaContext.java (patches/llama.rn+0.4.8.patch) to also catch:
//      - Build.SOC_MODEL.startsWith("TENSOR") — any Pixel Tensor chip
//      - Build.HARDWARE codenames: zuma, zumapro, ripcurrent, husky,
//        shiba, komodo, caiman, tokay — Pixel 8/9/10 board names
//      - Build.MODEL "Pixel 8 / 9 / 10" prefix — covers any future
//        chip/board codename
//      Blocklist hits force the v8_4_fp16_dotprod variant (compiled
//      WITHOUT +sve), which is the equivalent of upstream's
//      -DGGML_SVE=OFF compile flag. This is THE FIX per the upstream
//      llama.cpp investigation (issues #6995 + #8109). Pewter Vault
//      (OTA-273) already covers SD865/SD7xx/Exynos 990; this adds
//      Tensor G3/G4/G5.
//
//   2. QWEN THREADS DEFAULT 4 → 1. Per upstream advice ("Llama.cpp
//      sometimes crashes if you assign more physical cores than you
//      have") — single-threaded inference eliminates worker-pool
//      races entirely. Perf cost is real (~2-3x slower narration)
//      but on chips that we're already stepping down via blocklist,
//      the extra stability is worth more than the speed. Modern
//      flagships still go single-threaded; we lose some throughput
//      everywhere but eliminate the only Qwen failure mode we don't
//      have a code-level patch for.
//
//   3. ARBITER TEMPLATE LIBRARY TRIPLED. The valuable half of the
//      reverted Brass Helm. arbiter-intent-quotes.json: 10
//      categories × 7-8 lines → 24 lines each (~160 new lines).
//      arbiter-mood-quotes.json: 6 categories × 11-12 lines → ~35
//      lines each (~144 new lines). All original lines kept verbatim;
//      new lines added at the bottom of each array. Players on auto-
//      disabled Qwen (or whose Qwen falls back to templates on a
//      blocklist hit) see ~3x the variety. JSON is pure additive
//      data — no boot path involvement (these were NOT the
//      suspected freeze culprit; the mid-use crash detector +
//      mlHealth Promise.all expansion was).
//
//   EXPLICITLY OUT (the suspected boot-freeze culprit, NOT shipped):
//     - Mid-use crash detector (mlHealth.ts Promise.all 4 → 7 keys +
//       new midUseCrashCount path)
//     - gameStore.ts wrap of qwen.stream with markMLGenerate
//       attempt/success breadcrumbs
//     - Mire Coil's RE-DOWNLOAD QWEN MODEL button (not needed once
//       the real fix is in)
//
//   Pixel 10 Pro XL test bench: player will sideload the APK,
//   verify Qwen narration runs without crashes, then promote the
//   AAB to Play Console internal testing. The blocklist forces the
//   SVE-free variant; if that variant still crashes, we have a much
//   bigger upstream problem and would need to step Pixel down to
//   v8_2_fp16_dotprod next.
//
//   Files: NEW node_modules/llama.rn/.../LlamaContext.java edits via
//          patches/llama.rn+0.4.8.patch (regenerated; SHA changed
//          from Pewter Vault version),
//          app/ai/generation/QwenGenerativeEngine.ts (threads default
//          4 → 1; comment updated),
//          app/data/lore/arbiter-intent-quotes.json (re-tripled;
//          ~160 new lines),
//          app/data/lore/arbiter-mood-quotes.json (re-tripled;
//          ~144 new lines),
//          app/buildCodename.ts (Thorn Vault added),
//          app/buildInfo.ts (OTA-291 bump + this change note),
//          docs/build-codenames.md (Thorn Vault moved to current;
//          pool renumbered),
//          HANDOFF.md (closed issue entry).
export const OTA_BUILD_ID = '2026-06-02-291';
