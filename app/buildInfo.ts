...
// 2026-07-02-949 — craft recipe resolution now folds hyphens/punctuation to spaces on
// both sides. The input normalizer strips hyphens ("Aether-Reinforced Armor" → "aether
...
// 2026-07-03-956 — scanners no longer red-✗ each other in the inventory. All three
// scanners (Pulse / Aetheric / Mud) share the single off-hand equip slot, so equipping
// one marked the other two "slot taken" — making it look like you can only carry one.
// But the 4-slot tool belt holds all three AND each fires from the pouch. The inventory
// ✗ no longer fires for a pouch-eligible tool while the belt has room, so you can stow
// one of each. tsc app-clean. JS-only → OTA-safe.
// 2026-07-03-957 — HUNGER REMOVED. The hunger stat (hungerStaminaPenalty, 0–5) was a
// hidden, unexplained mechanic whose only effect was shrinking the usable stamina cap;
// it grew +1 per 8 in-game hours (every rest ticked it) and food that reset it restored
// 0 stamina (measured against the hungry cap). Food already tops off HP and water tops
// off stamina, so it just added invisible friction. effectiveStaminaMax now keeps the
// engine bonusStaminaMax but drops the hunger term; nothing accrues the penalty; stale
// saves load at 0; hunger lines self-suppress. JS-only → OTA-safe.
// 2026-07-03-958 — Climbing Rope + Reclaimer's Rope are gear, not relics. They were
// kind:'relic' (a NON_TOOL_KIND), so the inventory filed them under RELICS and the
...
// 2026-07-04-960 — persistent "current mission" line (ported from Tartaria OTA-668). A
...
// 2026-07-04-962 — uniform ACTIVATE / DEACTIVATE (pause) across EVERY contract kind (ported
// from Tartaria OTA-670). Before this only faction quests could be paused; hunts, mysteries,
...
// 2026-07-04-963 — stumbled-onto missions now ANNOUNCE + offer accept/decline instead of
// silently committing you (ported from Tartaria OTA-671). The Parley of Factions was the
// offender: approaching the leaders auto-created the broker contract with no consent. Now
// handleBroker raises a pendingMissionOffer (announce + demands preview); a BrandedModal asks
...
// 2026-07-04-965 — make the mission line SLAP (ported from Tartaria OTA-673). The standing
// reminder now rides its own yellow 'mission' log channel with a MISSION chip, and the mission
// NAME ("Broker an Alliance") renders in a brighter accent yellow. The stumbled-onto parley
...
// JS-only → OTA-safe.
// 2026-07-04-966 — close the building-room save/exit/rehydrate material FARM (ported from
// Tartaria OTA-674). Inside a building, each room's cleared (take/salvage/investigate) state
// was keyed by the VOLATILE outdoor micro-micro id, which re-rolls when the tile's scene
// rebuilds after a reload — so re-entering refilled every tab. patchSceneForBuildingRoom now
...
// 2026-07-04-968 — ropes/tools scrap+repair with their real material (ported from Tartaria
// OTA-676). repairCostMaterials = scrap × 2, and scrapEngine didn't recognize `rope` as
// fiber, so a rope-tagged item (and the metal-less Pry Bar) fell through to the bare
...
// 2026-07-04-972 — broaden fuse material mapping (ported from Tartaria OTA-680). inferGearTagPack's
...
// 2026-07-05-977 — dog takes combat damage again + dog-vest equipped marker (ported from Tartaria
...
// 2026-07-05-979 — vendor sell list excludes equipped gear by INSTANCE ID, not name (ported from
// Tartaria OTA-687). The old name filter hid every copy sharing an equipped item's name, so a spare
// couldn't be sold without unequipping. New equippedInstanceIds(player) returns the exact worn
// instances (id-bound + legacy fallback, incl. ring2/ring3); the sell filter excludes those ids,
// leaving spares sellable. New equippedInstanceIds suite (4). JS-only → OTA-safe.
// 2026-07-05-980 — fused armor category fix + magical ✶ marker (ported from Tartaria OTA-688; the
// forge-complete message already named the kind correctly here). categorizeItem trusts a fused
// item's KIND so a fused ARMOR no longer mis-files under Weapons (which broke the "View in
...
// 2026-07-05-982 — bandolier bug fixes (from Tartaria OTA-690's bandolier pass; engine_Dev already
// had coatings-eligible + its own thrown-coating). (1) "Only one throwable" fixed: stowInBandolier
// now racks the next UN-racked instance, so several same-named throwables each take their own loop.
...
// 2026-07-06-988 — grammar (ports Tartaria OTA-697): the golem-kill line read "crumbles under
// the Bob's assault" — an article before the golem's proper name. Every other golem line treats
// the name as a bare noun ("Bob attacks", "Bob lands …"), so drop the "the": "crumbles under
// Bob's assault". JS-only → OTA-safe.
// 2026-07-06-989 — three playtest glitches (ports Tartaria OTA-698/699). (1) CRYSTAL HOOK
// DEAD-END: `take <hook noun>` now falls through to advance an unresolved (non-puzzle) hook when
// nothing's on the ground, and 'listen'/'hear' map to the investigate (hook-eligible) intent.
// (2) AMBIGUOUS NOUN RECENCY: resolveContextNoun prefers the noun the player most recently
// interacted with, so "the hatch" in a two-hatch room resolves to the one just touched.
// (3) ARBITER REPEAT: on-target room-flavor line now uses rotatingPick (anti-repeat) instead of
// plain pick(). New parser suite (5).
...
// 2026-07-06-994 — fused armor mis-named/mis-filed (ports Tartaria OTA-704). Qwen namer emitted the
// generic "Aetheric Armor" (also an authored runecaster WEAPON), so itemBackfill re-tagged the
...
// 2026-07-06-998 — every crafting tab shows a "makeable NOW" count (ports Tartaria OTA-708). CRAFT/
// RECIPES/<magic> each show (N) = what you can actually make with materials in hand (uses the LIVE
// content-pack recipe table); REPAIR badge switched to the affordable count. Helper + test.
// 2026-07-07-999 — grammar: "The the relic responds…" fallback fix (ports Tartaria OTA-709).
// 2026-07-07-1000 (OTA-1000) — scene-interaction dead-ends (ports Tartaria OTA-710). (1) TAKE infers
// an improvised weapon from a weapon-named scene noun ("take ice axe") instead of the salvage refusal.
// (2) SALVAGE falls back to the parser's context-resolved noun (leading article stripped so the line
// templates don't emit "The the reactor") so it works after investigate / on a location alias. (3)
...
// 2026-07-07-1002 (OTA-1002) — the OTA-1001 Aetherkin provoke is now fully DATA-DRIVEN (ports Tartaria
// OTA-712), closing the lore-in-engine-code gap for this content-pack build. The enemy to spawn, the
// corruption, the trigger nouns, and the narration all live in the encounter's `provoke` block in
// wasteland_encounters.json (generic: any type:'npc' archetype can carry one). The engine now holds NO
// encounter-specific enemy name or prose — it reads player.pendingProvoke (replaces the boolean
// aetherkinCoinPending) and spawns/narrates from the content. A content pack can author its own. +2 tests.
// 2026-07-07-1003 (OTA-1003) — auto-route travel slightly more eventful + more VARIED, not more fights
// (ports Tartaria OTA-713). On a plotted course the encounter roll chance nudges 0.55 → 0.58, and the
// non-combat archetypes (treasure / npc / fusion_bench) get a 1.3× weight bias so a route brings in more
// DIFFERENT encounters while the combat FRACTION drops a few points. Combat weight + enemy level
// untouched; cardinal travel unchanged. +2 tests. JS-only.
...
// 2026-07-07-1006 (OTA-1006) — bonus "good material" sprinkle (ports Tartaria OTA-716). ADDITIVE only:
// a hard-won fight (~22%) or a completed story thread (~40%) occasionally drops a bonus material on top
...
// OTA-1015 — vendors SELL recipes (gold sink, ports Tartaria OTA-726). Every trader teaches a small,
// stable per-vendor slice of the rare/legendary recipes you haven't learned (200 TC Rare / 500 TC
// Legendary). "buy <recipe name>" routes through the buy flow into knownRecipes (you still gather mats +
// forge it). Draws from the LIVE content-pack table (getRecipes) — lore-agnostic. Vendor greeting lists
// the workings for sale + reminds you any vendor mends worn gear ("repair <item>"). +2 tests.
// OTA-1016 — two more gold sinks (ports Tartaria OTA-728 + OTA-729), lore-agnostic: (1) PAY-TO-TRAIN —
// "train <stat>" pays TC to raise a stat +1; cost scales (50×) and caps at 15 so it's a sink not a cheat.
// (2) COMPANION CARE — "heal dog"/"heal <summon>" patches a companion to full (cost scales with missing
// HP); "revive dog" restores a dead/abandoned dog for 300 TC. Uses player.sidekick + getNarratorName() +
...
// OTA-1081 — DODGE reworked into an AC-bypass gamble; dog DISTRACT gets a scaled DC and a
...
// OTA-1082 — exploit-sweep fixes (multi-agent audit; shared across all three lines).
...
// OTA-1085 — punch-list A1+A2: small-bug closes + item-dupe closes. A1 SMALL BUGS:
// (1) enemy DOT statuses tick once per COMBAT ROUND, not only on `attack` (frozen
// while dodging/moving/commanding a companion; hoisted into runEnemyGroupCounters,
// attack path passes skipDotTick). (2) Hard training ceiling MAX_TRAINED_STAT=30
// on the player + dog (engine_Dev has no golem system). (3) `jump at <bogus text>`
// needs a RESOLVED scene noun to train DEX. (4) defeatedEnemies de-duplicated
// distinct-name set (self-heals legacy saves) so it can't grow unbounded → save
// bloat. (5) Wild-water re-arm moved off the bounceable per-scene flags onto
// worldMemory, keyed per source on game-hours (WATER_REARM_HOURS=6); the refusal
// lines use getNarratorName() (lore-neutral). A2 ITEM DUPES: (6) dropped-item
// PICKUP routes through grantItem (no laundering a worn/coated/rolled instance
// into a full same-name stack) + decrements the exact instance by id. (7)
...
// OTA-1087 — punch-list B1 (economy re-tiering). (a) self-crafted items never sell
// above their recipe ingredient value (Legendary +25%); (b) fused items stay
// scrappable but the fuel mats they yield (Golem Core, Aetheric Shard/Dust, Aether
// Crystal, Aetheric Cloth, Mudstone) price near-worthless at vendors (flat 3 TC) —
// crafting-only value, no fuse→scrap→sell pump; (c) nothing sells above the
// cheapest realistic buy for its rarity (RARITY_BUY_FLOOR), closing cross-stall
// arbitrage; (d) gifting a near-worthless item is declined (no rep/CHA, not
// consumed) and rep scales with the gift's worth instead of a flat +5. All
// lore-neutral (the gift decline names the vendor, not the Arbiter).
// OTA-1088 — GIFTING REMOVED. Faction standing is earned through mission
// completions + sigil/pendant turn-ins; gifting vendors loot for rep undercut that
// design and was undiscoverable, so the whole mechanic is gone (the `gift` Intent,
// parser verb + frame, giftToVendor action + case handler, LLM-parser row, and the
// stray gift mentions in help/hint/modal text). 1087's B1d gift value-gate is
// superseded. The buy-for-rep +1 side door is left as-is.
// OTA-1089 — BUY-FOR-REP reduced to a slow afterthought. Was a flat +1 standing
// per purchase; now standing accrues by TC of honest custom (+1 per 500 TC spent,
...
// OTA-1094 — PARLEY Phase 2 (procedural payloads + dialogue beats + real rewards).
...
// OTA-1096 — PROSE POLISH: remove command-syntax placeholders from narration
...
// unfilled templates in the prose; rewritten in plain language (keeps "sidekick" for
// lore-neutrality). Same fix on the three narrator "send word <name>" courier lines →
// "send word by name".
// OTA-1097 — READABILITY: (1) the same-channel feed debounce now joins grouped
// world/system beats with a PARAGRAPH BREAK ("\n\n") instead of two spaces (no more
...
//
// OTA-1099 — two playtest bugs (device-log re-read):
...
// OTA-1101 — REGULAR-ENEMY SCALING. Base enemy stats came straight from enemies.json,
// so an over-leveled character farmed trivial trash. scaledEnemyForContext
// (encounter.ts) lifts a NON-BOSS enemy's HP (and lightly its attack/AC) by player
// power AND tile danger: a rising HP floor (maxed player ~34 at danger 0 to ~64 at
// danger 5) plus a flat-capped multiplier (a big Legendary is nudged, not exploded).
// A fresh arrival on a danger-0 tile is untouched. The data-driven quest boss is
// skipped (bosses carry their own curve). Applied at scene arrival + groups,
// rest-ambush, and hook spawn.
// OTA-1102 — MIXED-ROLE PACKS + PACK-AWARE SCALING. (1) The curated single-enemy path
...
// OTA-1104 — THEMATIC ("Pokémon-route") weakness + medium hardness + diegetic read.
// 1103's flat-random weakness read as nonsense (mud weak to cold). Now each creature
// TYPE has a small pool of plausible weaknesses (THEMATIC_DEFENSE_POOLS) and a spawn
// rolls ONE — believable every time, varied fight to fight. MEDIUM hardness: weakness is
// a ×1.5 bonus, everything else stays normal & still works (never hard-locked); one soft
...
// OTA-1107 — VENDOR RECIPE MENU: NO REROLL + NO "BUY & EQUIP". Buying a recipe rerolled
...
// OTA-1108 — NARRATOR "theYou" GLUE + TORCH-LEAD CLARITY. (1) Qwen 0.5B sometimes emits
...
// ── OTA-1129 (Character screen — readability: expandable stats, AC breakdown, title provenance) ─
// Three tap-to-expand upgrades to the Character sheet (engine teal palette on the
// new rows; custom/importable titles also get provenance):
...
//   • TITLES — each title is singly tappable: shows HOW it was earned (the
//     requirement) and, for earned titles, WHEN — a new player.titleLog captures
//     the in-game hour + real timestamp at award (awardNewTitles, built-in AND
//     custom titles), rendered "Day N (period) · YYYY-MM-DD" by the tested
//     describeTitleEarned helper; pre-tracking titles get an honest fallback.
// Test: ota848TitleProvenance.
// ── OTA-1130 (LIVING WORLD — tides get teeth + World view + raids + kill-standing + offline) ─
// Makes OTA-844's World Pulse live instead of cosmetic (engine line: WorldScreen uses
// getFactions() + teal palette). New WORLD view (from the Character sheet's faction
// section) shows every faction's momentum meter + your standing + the rumour feed
// (fixes tide invisibility). Vendor prices move with the vendor's faction tide
// (tideVendorPriceMult, ±20%). Rival factions RAID a player who's taken a side — the
// most-ascendant eligible rival drops a war party into a peaceful outdoor scene
// (maybeSpawnRaid; cooldown-gated, avoidable, party size ESCALATES with tide). Killing
...
// ── OTA-1133 (surface WORLD + LORE on the crest; fix the crowded Lore tab row) ──
// Port of HAL OTA-852 (engine teal crest buttons). The idle peaceful crest square
...
//
// 2026-07-16 OTA-1141 — port of HAL OTA-860/861. First-time tips pass: added a ~25-word
...
//
// 2026-07-16 OTA-1144 — port of HAL OTA-864. War-feed flavour: the World board drew patrol
...
//
// 2026-07-16 OTA-1145 — port of HAL OTA-865. War micro-economy: local war heat (from
...
//
// 2026-07-16 OTA-1146 — port of HAL OTA-866. Every accepted bounty on the Missions board now
...
//
// 2026-07-18 OTA-1151 — port of HAL OTA-873 (Crucible "Upgrade" mode). A 4th Crucible mode
// beside Weapon / Armor / Dog: pick exactly 5 reserved pieces, then choose a piece to work
// an extra coating channel into it. WEAPON → a 2nd coating slot (coatingSlots→2; both coatings
// fire on every landing hit — two roll steps, two DOTs). ARMOR / DOG-VEST → +1 worked-in-resist
// capacity (resistCapBonus→1; holds one more resist than the stock cap of 3). Adds a coating
// channel only — no AC / damage change. New store action upgradeCoatingSlot; coating2 +
...
// 2026-07-18 OTA-1151c — hardening: a 5-agent pressure-test suite found two upgradeCoatingSlot
// exploits, fixed here. (1) dup-id cost bypass — passing the same reserved id five times
// satisfied the ===5 gate but consumed only one unit; fixed by deduping itemIds (kept engine's
// isFusionReservable). (2) stack mass-upgrade — a quantity>1 target upgraded every copy for one
// cost; fixed by refusing a stacked target. Two EXPLOIT-GUARD regression tests; suite 11/11.
// 2026-07-18 OTA-1151d — armor-coating hygiene: acid/corruption vials worked into ARMOR
...
// 2026-07-19 OTA-1160 — missions show distance in moves + sort-by-distance (port of HAL OTA-883).
// Every mission entry (bounties, hunts, mysteries, storylines, faction quests, alliance broker
// legs, whispers, leads, sigils) shows "◈ N moves away" to its target, and a SORT BY DISTANCE
// toggle at the top of the missions scroll reorders entries nearest-first while keeping them
// GROUPED BY TYPE (sorted within each section; placeless entries last). Distance =
...
// 2026-07-19 OTA-1165 — First Aid Kit 'Heal to full' option (port of HAL OTA-888). The inventory
...
// stays as a de-emphasised secondary. Engine keeps its earlier perItemHP/stackQty declaration site.
// JS-only; ships over-the-air. Covered by healBatch.test.ts.
// 2026-07-19 OTA-1166 — stealth day/night made consistent + visible (port of HAL OTA-889). The +1-
...
// 2026-07-19 OTA-1168 — signature-weapon leave line deduped per scene (port of HAL OTA-891). An
...
// 2026-07-19 OTA-1169 — USE TORCH back in the primary quick row (port of HAL OTA-892). Player ask:
...
// toggle); the menus + rarer actions stay behind MORE. Engine keeps its amber "all missions" label
// in the tray. UI-only; ships over-the-air.
// 2026-07-19 OTA-1170 — narration consistency audit sweep (port of HAL OTA-893). Six-lens pass over
...
// enemy, an indefinite-article sweep (an Rare weapon / A Eternal Dynasty patrol / {enemy} wasteland /
// scene noun / ambient-flavor / way-forward / arrival cap), definite-article title-doubling ("the The
// …") via theLower() across quest sites + broker + echo, "flares"/"sets in" capitalization, plural
