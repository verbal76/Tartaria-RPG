...
//
// ⚠⚠ OTA-1255 — 'look' ADDED, AND THE LIST MOVED HERE (ported from golem
// OTA-1249; the picker work around it stays golem-only). Owner: *"you should type
// your name, then get the prompt for look around you, and it should be the only
// button highlighted... the look around you button fix can go to Hal."* It was not
// the only one — it was one of eleven live buttons, because 'look' was the one
// beat between 'name' and 'explore_or_leave' missing from this list, so the
// lockdown simply switched off for the length of it. The beat lit LOOK AROUND YOU
// green and left travel, investigate, take, salvage, craft, inventory and the rest
// tappable beside it.
//
// ⚠ AND IT LIVED IN THREE PLACES — identical literal arrays in InputBox, in
// ExplorationScreen, and in the store's typed-input gate. A rule computed three
// times drifts. One export, three readers.
