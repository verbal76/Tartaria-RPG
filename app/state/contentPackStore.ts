...
interface ContentPackState {
  ...
  wasteland: Record<string, unknown>;
  /** Uploaded scene-prop pools ({ climbables?: [...], salvageables?: [...] }). Empty = built-in. */
  sceneProps: ScenePropsOverride;
  ...
  setEnergyName: (name: string) => void;
  ...
  /** Parse a scene-props object (JSONC): { climbables?: [{name,context?,height?}],
   *  salvageables?: [{name,context?}] }. Replaces the built-in curated climb/salvage
   *  noun pools the engine injects into scenes. */
  loadScenePropsJson: (json: string) => BundleLoadResult;
  /** Parse a named-vendors array (JSONC). Each: { id, name, title, faction?, description,
  ...
  clearDigging: () => void;
  loadScrapJson: (json: string) => LoadResult;
  clearScrap: () => void;
  ...
  clearWasteland: () => void;
  /** Drop the uploaded scene-prop pools back to the built-in set. */
...
export const useContentPackStore = create<ContentPackState>((set, get) => ({
  ...
  reapply() {
    ...
    setEnergyNameOverride(s.energyName || null);
  ...
  setEnergyName(name) {
    const clean = name.trim();
    setEnergyNameOverride(clean.length > 0 ? clean : null);
    set({ energyName: clean, contentVersion: get().contentVersion + 1 });
    persist({ ...get(), energyName: clean });
  },
  ...
  loadScenePropsJson(json) {
    ...
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
      return { ok: false, error: 'Scene props must be a JSON OBJECT: { "climbables": [...], "salvageables": [...] }.' };
    }
    const obj = parsed as { climbables?: unknown; salvageables?: unknown };
    const cleanPool = (v: unknown, withHeight: boolean): ScenePropSpawn[] => {
      if (!Array.isArray(v)) return [];
      return v
        .filter((r): r is Record<string, unknown> => !!r && typeof r === 'object' && typeof (r as { name?: unknown }).name === 'string' && ((r as { name: string }).name).trim().length > 0)
        .map((r) => {
          const ctx = r.context === 'inside' || r.context === 'outside' || r.context === 'both' ? r.context : 'both';
          const out: ScenePropSpawn = { name: (r.name as string).trim(), context: ctx };
          if (withHeight && typeof r.height === 'number' && r.height > 0) out.height = Math.min(8, Math.round(r.height));
          return out;
        });
    };
    const climbables = cleanPool(obj.climbables, true);
    const salvageables = cleanPool(obj.salvageables, false);
    if (climbables.length === 0 && salvageables.length === 0) {
      return { ok: false, error: 'No props found. Provide "climbables" and/or "salvageables" arrays of { "name", "context"?, "height"? }.' };
    }
    const sceneProps: ScenePropsOverride = { climbables, salvageables };
    setScenePropsOverride(sceneProps);
    set({ sceneProps, contentVersion: get().contentVersion + 1 });
    persist({ ...get(), sceneProps });
    return { ok: true, summary: `Loaded ${climbables.length} climbable + ${salvageables.length} salvageable scene prop(s).` };
  },

  loadVendorsJson(json) {
    let parsed: unknown;
    try {
      parsed = JSON.parse(stripJsonComments(json));
    } catch (e) {
    ...
    // Accept a bare array OR an object wrapped under "vendors".
    const arr = Array.isArray(parsed)
      ? parsed
      : parsed && typeof parsed === 'object' && Array.isArray((parsed as { vendors?: unknown }).vendors)
        ? (parsed as { vendors: unknown[] }).vendors
        : null;
    if (!arr) {
      return { ok: false, error: 'Vendors must be a JSON ARRAY (or an object with a "vendors" array) of trader rows.' };
    }
    const valid = arr.filter(
      (v) => !!v && typeof v === 'object' && typeof (v as { name?: unknown }).name === 'string' && Array.isArray((v as { offers?: unknown }).offers),
    );
    if (valid.length === 0) {
      return { ok: false, error: 'No vendors found. Each needs at least { "name", "offers": [{ "itemName", "price" }] }.' };
    }
    setVendorsOverride(valid);
    const vendors = valid;
  ...
  setWeatherEnabled(on) {
    setWeatherEnabledEngine(on);
    set({ weatherEnabled: on, contentVersion: get().contentVersion + 1 });
  ...
  loadScrapJson(json) {
    let parsed: unknown;
    try { parsed = JSON.parse(stripJsonComments(json)); } catch (e) { return { ok: false, error: `Not valid JSON: ${e instanceof Error ? e.message : String(e)}` }; }
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return { ok: false, error: 'Scrap must be a JSON OBJECT: { roles, rawGuard, premiumMats, failureLines }.' };
    const obj = parsed as Record<string, unknown>;
    if (!obj.roles || typeof obj.roles !== 'object') return { ok: false, error: 'Scrap needs a roles{} object (role → material name).' };
    setScrapOverride(obj);
    set({ scrap: obj, contentVersion: get().contentVersion + 1 });
    persist({ ...get(), scrap: obj });
    return { ok: true, count: Object.keys(obj.roles as object).length };
  },
  clearScrap() {
    setScrapOverride(null);
    set({ scrap: null, contentVersion: get().contentVersion + 1 });
    persist({ ...get(), scrap: null });
  },
  loadSalvageJson(json) {
    let parsed: unknown;
    try { parsed = JSON.parse(stripJsonComments(json)); } catch (e) { return { ok: false, error: `Not valid JSON: ${e instanceof Error ? e.message : String(e)}` }; }
    if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) return { ok: false, error: 'Salvage must be a JSON OBJECT: { pools: [...], junkPool, materialLines, junkLines, nothingChance }.' };
    const obj = parsed as Record<string, unknown>;
    if (!Array.isArray(obj.pools) || obj.pools.length === 0) return { ok: false, error: 'Salvage needs a non-empty pools[] array.' };
    setSalvageOverride(obj);
    set({ salvage: obj, contentVersion: get().contentVersion + 1 });
    persist({ ...get(), salvage: obj });
    return { ok: true, count: (obj.pools as unknown[]).length };
  },
  clearSalvage() {
    setSalvageOverride(null);
  ...
  loadGameBundle(json, opts) {
    ...
    let nextEnergyName = get().energyName;
    const applied: string[] = [];
    const skipped: string[] = [];
    ...
    for (const [key, value] of Object.entries(obj)) {
      ...
      if (key === 'missions') {
      ...
      } else if (key === 'sceneProps') {
        if (value && typeof value === 'object' && !Array.isArray(value)) { nextSceneProps = value as ScenePropsOverride; applied.push('scene props'); }
        else skipped.push('sceneProps (not an object)');
      } else if (key === 'vendors') {
        ...
        if (value && typeof value === 'object' && !Array.isArray(value) && (value as { roles?: unknown }).roles) { nextScrap = value as Record<string, unknown>; applied.push('scrap'); }
        else skipped.push('scrap (needs an object with roles{})');
      } else if (key === 'salvage') {
      ...
      } else if (key === 'energyName' || key === 'energy_name') {
        if (typeof value === 'string') { nextEnergyName = value.trim(); applied.push('energy name'); }
      } else {
    ...
    setEnergyNameOverride(nextEnergyName || null);
    set({
      tables: nextTables,
      lore: nextLore,
  ...
  exportGameBundle() {
    ...
    if (s.energyName) out.energyName = s.energyName;
  ...
  clearSceneProps() {
    ...
    const sceneProps: ScenePropsOverride = {};
    set({ sceneProps, contentVersion: get().contentVersion + 1 });
    persist({ ...get(), sceneProps });
  },
  ...
  async hydrate() {
    try {
      ...
      if (raw) {
        ...
        const sceneProps: ScenePropsOverride = shape.sceneProps && typeof shape.sceneProps === 'object' ? shape.sceneProps : {};
        setScenePropsOverride(sceneProps);
        ...
        const scrap = shape.scrap && typeof shape.scrap === 'object' ? (shape.scrap as Record<string, unknown>) : null;
        ...
        const energyName = typeof shape.energyName === 'string' ? shape.energyName : '';
