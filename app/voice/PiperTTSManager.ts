...
// ⚠⚠ OTA-1356 — THE VENDOR-LOAD COOLDOWN, born from two tombstones. Freeze #4's
...
async function ensureLoaded(voiceId: string): Promise<any | null> {
  ...
  // ⚠ OTA-1356 — the cooldown gate (see the block above VENDOR_LOAD_COOLDOWN_MS).
  ...
  const promise = (async () => {
    ...
    try {
      ...
      // ⚠⚠ OTA-1356 — THE LOAD JOINS THE NATIVE-ML LOCK. arb159 put every
      ...
      // ⚠ OTA-1356 — a failed load, whatever the step, means the device just
