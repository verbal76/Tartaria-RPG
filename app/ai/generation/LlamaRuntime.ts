...
export interface LlamaGenerateOptions {
  ...
  /** ⚠⚠ OTA-1359 — THE DOOR CHECK. "Is this output still wanted?", asked
...
export class LlamaRuntime {
  ...
  async generate(
    ...
    try {
      ...
      let preempted = false;
      let stopAsked = false;
      // OTA-1359 — see `shouldAbort`. A throwing predicate must never cost a
      ...
      const onTokenEvt = (opts.onToken || opts.shouldAbort)
        ? (evt: LlamaTokenEvent) => {
            ...
            // OTA-1359 — the SECOND door check: this generation went stale
      ...
      const result = await runExclusiveNativeMl(() => {
        telLockAt = Date.now();
        // ⚠⚠ OTA-1359 — THE DOOR. The lock is ours and the native call has
