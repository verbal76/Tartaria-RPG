...
export class LlamaRuntime {
  ...
  async initialize(opts: LlamaInitOptions): Promise<void> {
    ...
    // ⚠ OTA-1353 — bracket the ~425MB native open on the dying-breath crumb. A
    ...
    try {
      // eslint-disable-next-line @typescript-eslint/no-require-imports
      (require('../../engine/saveSystem') as typeof import('../../engine/saveSystem')).stampBreadcrumbPhase('ctx-open-done'); // OTA-1353
    } catch { /* ignore */ }
  ...
  async dispose(): Promise<void> {
    ...
    // ⚠ OTA-1353 — bracket the native free too: the third B9 freeze died 10s
    ...
    try {
      ...
      try {
        // eslint-disable-next-line @typescript-eslint/no-require-imports
        (require('../../engine/saveSystem') as typeof import('../../engine/saveSystem')).stampBreadcrumbPhase('ctx-release-done'); // OTA-1353
      } catch { /* ignore */ }
    } catch {
