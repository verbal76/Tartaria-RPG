...
`mac_dev` stopped here. **Nothing has been deleted.** Every commit and every
line of code on this branch is exactly where it was; what ended is its future,
not its past.

| | |
|---|---|
| **last independent commit** | `6c4a5fb2` |
| **what it was** | the standing branch for the macOS .dmg build |
| **what builds that now** | `golem-line`, the trunk, via `.github/workflows/build-mac.yml` |
...
trunk, not from here** — `.github/workflows/build-mac.yml` triggers on `golem-line`. That has been
...
routing rule, and `docs/WORKFLOWS.md` for how macOS .dmg is actually built and
shipped.
